...
class block(object):
    ...
    def tag(cls, bb):
        ...
        col, _ = interface.function.blockcolor(bb), res.pop('__color__', None)
    ...
    def tag(cls, bb, key, value):
        ...
        if key == '__color__':
            return interface.function.blockcolor(bb, value)
    ...
    def tag(cls, bb, key, none):
        ...
        if key == '__color__':
            DEFCOLOR = 0xffffffff
            return interface.function.blockcolor(bb, DEFCOLOR)
...
class type(object):
    ...
    def outline(cls, func):
        ...
        return True if cls.flags(func, FUNC_OUTLINE) else False
    @utils.multicase(func=(idaapi.func_t, types.integer))
    @classmethod
    ...
    class argument(object):
        ...
        def __new__(cls, func, index):
            ...
            if not (0 <= index < ftd.size()):
                raise E.IndexOutOfBoundsError(u"{:s}({:#x}, {:d}) : The provided index ({:d}) is not within the range of the number of arguments ({:d}) for the specified function ({:#x}).".format('.'.join([__name__, cls.__name__]), ea, index, index, ftd.size(), ea))
        ...
        def __new__(cls, func, index, info):
            ...
            ti, ftd = builtins.next(updater)
            if not (0 <= index < ftd.size()):
                raise E.IndexOutOfBoundsError(u"{:s}({:#x}, {:d}, {!r}) : The provided index ({:d}) is not within the range of the number of arguments ({:d}) for the specified function ({:#x}).".format('.'.join([__name__, cls.__name__]), ea, index, "{!s}".format(info), index, ftd.size(), ea))
        ...
        def storage(cls, func, index):
            ...
            if tinfo is None:
                _, ea = interface.addressOfRuntimeOrStatic(func)
                raise E.DisassemblerError(u"{:s}.storage({:#x}, {:d}) : Unable to get the prototype for the specified function ({:#x}).".format('.'.join([__name__, cls.__name__]), ea, index, ea))
        ...
        def location(cls, ea, index):
            ...
            if not (interface.xref.has_code(ea, descend=True) and interface.instruction.is_call(ea)):
                raise E.InvalidTypeOrValueError(u"{:s}.location({:#x}, {:d}) : Unable to return any parameters as the provided address ({:#x}) {:s} code references.".format('.'.join([__name__, cls.__name__]), ea, index, ea, 'does not have any' if interface.instruction.is_call(ea) else 'is not a call instruction with'))
            ...
            raise E.IndexOutOfBoundsError(u"{:s}.location({:#x}, {:d}) : Unable to fetch the address of the specified parameter ({:d}) from the function call at address {:#x} due to only {:d} parameter{:s} being available.".format('.'.join([__name__, cls.__name__]), ea, index, index, ea, len(parameters), '' if len(parameters) == 1 else 's'))
    ...
    class arguments(object):
        ...
        def iterate(cls, func):
            ...
            if tinfo is None:
                _, ea = interface.addressOfRuntimeOrStatic(func)
                raise E.DisassemblerError(u"{:s}.iterate({:#x}) : Unable to get the prototype for the specified function ({:#x}).".format('.'.join([__name__, cls.__name__]), ea, ea))
        ...
        def storage(cls, func):
            ...
            if tinfo is None:
                _, ea = interface.addressOfRuntimeOrStatic(func)
                raise E.MissingTypeOrAttribute(u"{:s}.storage({:#x}) : Unable to get the prototype for the specified function ({:#x}).".format('.'.join([__name__, cls.__name__]), ea, ea))
    ...
    def lumina(cls, func):
        ...
        fn = interface.function.by(func)
        return True if interface.function.flags(fn, getattr(idaapi, 'FUNC_LUMINA', 0)) else False
    @utils.multicase(func=(idaapi.func_t, types.integer))
    @classmethod
