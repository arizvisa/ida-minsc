...
class imports(object):
    ...
    def has(cls, ea):
        ...
        return idaapi.segtype(ea) == idaapi.SEG_XTRN and any(address == ea for ea, _ in cls.__iterate__())
...
class types(object):
    ...
    @classmethod
    def get(cls, info):
        '''Get the type referenced by the type in `info` from the current type library and return it.'''
    ...
    def get(cls, info, library):
        '''Get the type referenced by the type in `info` from the specified type `library` and return it.'''
    ...
    def add(cls, name, info, library, **mangled):
        ...
        if error == idaapi.TERR_WRONGNAME:
            raise E.DisassemblerError(u"{:s}.add({!r}, {!r}, {:s}{:s}) : Unable to add a type with the name \"{:s}\" to the type library due to error {:s}.".format('.'.join([__name__, cls.__name__]), name, "{!s}".format(info), interface.tinfo.format_library(library), u", {:s}".format(utils.string.kwargs(mangled)) if mangled else '', utils.string.escape(identifier, '"'), interface.tinfo.format_type_error(error) or "code ({:d})".format(error)))
        raise E.DisassemblerError(u"{:s}.add({!r}, {!r}, {:s}{:s}) : Unable to add the given type to the type library due to error {:s}.".format('.'.join([__name__, cls.__name__]), name, "{!s}".format(info), interface.tinfo.format_library(library), u", {:s}".format(utils.string.kwargs(mangled)) if mangled else '', interface.tinfo.format_type_error(error) or "code ({:d})".format(error)))
