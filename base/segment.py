...
@document.aliases('range')
@utils.multicase()
@document.parameters(segment='an identifier used to specify a segment')
def bounds(segment):
...
@utils.multicase(ea=six.integer_types)
@document.parameters(segment='an identifier used to specify a segment', ea='an address within the segment')
...
@document.aliases('byoffset')
@utils.multicase(offset=six.integer_types)
@document.parameters(offset='the offset into the current segment to return as an address')
...
@document.aliases('byoffset')
@utils.multicase(offset=six.integer_types)
@document.parameters(segment='an identifier used to specify a segment', offset='the offset into the segment to return as an address')
...
@document.aliases('goof',' gooffset', 'gotooffset')
@utils.multicase(offset=six.integer_types)
@document.parameters(segment='an identifier used to specify a segment', offset='an offset into the segment')
...
@utils.multicase(none=None.__class__)
@document.parameters(segment='an identifier used to specify a segment', none='the python value `None`')
...
@document.parameters(segment='an identifier used to specify a segment', rgb='the color as a red, green, and blue integer (``0x00RRGGBB``)')
...
@utils.multicase(ea=six.integer_types)
@document.parameters(address='an address belonging to the segment that the type should be returned for')
def type(ea):
...
@utils.multicase(name=six.string_types)
@utils.string.decorate_arguments('name')
@document.parameters(name='the name of the segment to return the type for')
def type(name):
    '''Return the type of the segment with the specified `name`.'''
    seg = by_name(name)
    return type(seg)
@utils.multicase(segment=idaapi.segment_t)
@document.parameters(segment='an `idaapi.segment_t` to return the type for')
def type(segment):
    '''Return the type of the ``idaapi.segment_t`` specified by `segment`.'''
    return segment.type
@utils.multicase(segtype=six.integer_types)
@document.parameters(segment='an `idaapi.segment_t` to check the type for', segtype='an integer containing the `idaapi.SEG_*` value to check the type against')
...
@document.parameters(segment='an identifier used to specify a segment', contents='whether to remove the contents of the segment from the database')
