...
class AArch(internal.architecture.architecture_t):
    ...
    def __init__(self, BITS):
        ...
        # Condition codes (not really registers, but condition_t)
        [ setitem("cc{:s}".format(_), self.pseudoregister("cc{:s}".format(_), 1, condition_t, index=index)) for index, _ in enumerate(['EQ', 'NE', 'CS', 'CC', 'MI', 'PL', 'VS', 'VC', 'HI', 'LS', 'GE', 'LT', 'GT', 'LE', 'AL', 'NV']) ]
    ...
    def by_condition(self, index):
        ...
        return getattr(self.register, "cc{:s}".format(cc[index]))
