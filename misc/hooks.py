...
class naming(changingchanged):
    @classmethod
    def is_autogenerated(cls, ea, name):
        '''Return whether the `name` at the address `ea` is an auto-generated label.'''
        prefix, address = name.split('_', 1) if '_' in name else ('', name)
        return prefix in {'loc', 'sub', 'byte'}
    ...
    def updater(cls):
        ...
        if (ea, expected) != (new_ea, expected and new_name):
            ...
            # If the name is an auto-generated one, then demote the loglevel.
            Flogging = logging.info if cls.is_autogenerated(new_ea, expected) else logging.fatal
        ...
        current_tags, current_count = context.get(new_ea, **kwds), context.count(new_ea, **kwds)
        oldname, old_tagged = '' if name_t == idaapi.FF_LABL else original, '__name__' in previous_tags
        newname, new_tagged = new_name if not(cls.is_autogenerated(ea, new_name)) else '', '__name__' in current_tags

        # If our tags haven't changed, then we're probably using the tag index.
        if previous_tags == current_tags:

            # If the name was changed from nothing to something, then increment.
            if not(oldname) and newname:
                context.increment(ea, '__name__', **kwds) if not(new_tagged) else ()
            ...
            # If the name was changed from something to nothing, then decrement.
                ...
                context.decrement(ea, '__name__', **kwds) if new_tagged else ()
            ...
            # we need to adjust it since we didn't catch the event.
            elif oldname and newname and not new_tagged:
