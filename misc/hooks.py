...
import internal, internal.tagindex
...
def __process_functions(percentage=0.10):
    ...
    for i, fn in enumerate(funcs):
        ...
        if fn not in globals and internal.tags.function.get(fn):
            [ internal.tags.reference.globals.increment(fn, k) for k in implicit if k in internal.tags.function.get(fn) ]
        ...
        for ci, (l, r) in enumerate(chunks):
            ...
            for ea in interface.address.items(ui.navigation.analyze(l), r):
                ...
                for k in available - implicit:
                    if ea in globals: internal.tags.reference.globals.decrement(ea, k)
...
class structures(changingchanged):
    ...
    def update_refs(cls, sid, old, new):
        ...
        oldkeys, newkeys, tags = ({item for item in content} for content in [old, new, internal.tagindex.structure.get(sid)])
        ...
        for key in oldkeys ^ newkeys:
            if key not in new and key in tags:
                ...
                internal.tagindex.structure.decrement(sid, key) if internal.tagindex.tags.has(key) else ()
    ...
    def create_refs(cls, sid, new):
        ...
        available = internal.tagindex.structure.get(sid)
        contentkeys = {item for item in new.keys()}
    ...
    def delete_refs(cls, sid, old):
        ...
        available = internal.tagindex.structure.get(sid)
        contentkeys = {item for item in old.keys()}
...
class structurenaming(changingchanged):
    ...
    def created(cls, struc_id):
        ...
        if not cls.is_general_name(sptr.id, name) and not sptr.props & idaapi.SF_NOLIST:
            internal.tagindex.structure.increment(sptr.id, '__name__')
        ...
        if cls.is_tracked(sptr.id, name):
            internal.tagindex.structure.increment(sptr.id, '__typeinfo__')
        return
    ...
    def deleted(cls, struc_id):
        ...
        erased, tags = [], internal.tagindex.structure.get(struc_id)
        ...
        if internal.structure.by_identifier(struc_id):
            mids = {mid for mid, _ in internal.tagindex.members.structure(struc_id)}
            erased = internal.tagindex.members.erase(struc_id, mids)

        removed = {sid for sid in internal.tagindex.structure.erase(struc_id)}
    ...
    def updater(cls):
        ...
        available = internal.tagindex.structure.get(newsid)

        # If we're switching from general to user-specified, then increment.
        if renamed == (True, False):
            internal.tagindex.structure.increment(newsid, '__name__') if '__name__' not in available else ()

        # If we're going from user-specified to general, then decrement.
        elif renamed == (False, True):
            internal.tagindex.structure.decrement(newsid, '__name__') if '__name__' in available else ()
...
class membertagscommon(changingchanged):
    ...
    def update_refs(cls, mid, old, new):
        ...
        oldkeys, newkeys, tags = ({item for item in content.keys()} for content in [old, new, internal.tagindex.members.get(mid)])
        ...
        for key in oldkeys ^ newkeys:
            if key not in new and key in tags:
                ...
                internal.tagindex.members.decrement(mid, key) if internal.tagindex.tags.has(key) else ()
            if key not in old and key not in tags:
    ...
    def create_refs(cls, mid, new):
        ...
        available = internal.tagindex.members.get(mid)
        contentkeys = {item for item in new.keys()}
...
class memberscopecommon(changingchanged):
    ...
    def delete_refs(cls, sid, mid):
        ...
        used = internal.tagindex.members.get(mid)
        [internal.tagindex.members.decrement(mid, key) for key in used if internal.tagindex.tags.has(key)]
...
class membernamingcommon(changingchanged):
    ...
    def updater(cls):
        ...
        sid, mid, available = newsid, newmid, internal.tagindex.members.get(newmid)
        ...
        if renamed == (True, False):
            internal.tagindex.members.increment(mid, '__name__') if '__name__' not in available else ()

        # Otherwise, the name was cleared to a default, which we'll decrement.
        elif renamed == (False, True):
            internal.tagindex.members.decrement(mid, '__name__') if '__name__' in available else ()
    ...
    def updater(cls):
        ...
        if (oldsid, oldmid) == (newsid, newmid):
            ...
            if applied == (False, True):
                internal.tagindex.members.increment(mid, '__typeinfo__') if '__typeinfo__' not in available else ()

            # If the change removed the type, then decrement the reference.
            elif applied == (True, False):
                internal.tagindex.members.decrement(mid, '__typeinfo__') if '__typeinfo__' in available else ()
        ...
        if '__typeinfo__' in available:
            internal.tagindex.members.decrement(mid, '__typeinfo__')
...
class localtypesmonitor_84(object):
    ...
    def delete_type(cls, sid):
        ...
        if tinfo.get_type_by_tid(sid):
            mids = {mid for mid, used in internal.tagindex.members.structure(sid)}
            erased = internal.tagindex.members.erase(sid, mids)
        ...
        removed = internal.tagindex.structure.erase(sid)
        if removed:
    ...
    def type_updater(cls, ltc, ordinal):
        ...
        # Grab the current state of the tags so we can log something useful.
            ...
            original = internal.tagindex.structure.get(newsid)
        ...
        modified = internal.tagindex.structure.get(newsid)
        ...
        for count, old, new in changes:
            if old and new:
                ...
                # Grab the tags that are currently applied to the member.
                    ...
                    original = internal.tagindex.members.get(mid)
                ...
                modified = internal.tagindex.members.get(mid)
            ...
            # non-general type or a user-specified name to update its tags.
                ...
                if not cls.is_field_general(ordinal, mindex, mname):
                    internal.tagindex.members.increment(mid, '__name__'), changed.append('__name__')

                if cls.is_field_tracked(ordinal, mindex, mtype):
                    internal.tagindex.members.increment(mid, '__typeinfo__'), changed.append('__typeinfo__')
