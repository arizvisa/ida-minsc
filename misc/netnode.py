...
class hash(object):
    ...
    def has(cls, nodeidx, key, tag=None):
        ...
        node, key_decoded = utils.get(nodeidx), cls.__decode_string(key)
        res = netnode.hashval(node, key_decoded, netnode.hashtag if tag is None else tag)
        return res is not None
    ...
    def fbounds(cls, nodeidx, start, stop, type=None, tag=None):
        ...
        if value_transform:
            node, [value, ok, transform] = utils.get(nodeidx), value_transform
            result = utils.fhashrange(node, start, stop, value, tag=netnode.hashtag if tag is None else tag)
            return [(hidx, transform(hval)) for hidx, hval in result] if transform else result
    ...
    def rbounds(cls, nodeidx, start, stop, type=None, tag=None):
        ...
        if value_transform:
            node, [value, ok, transform] = utils.get(nodeidx), value_transform
            result = utils.rhashrange(node, start, stop, value, tag=netnode.hashtag if tag is None else tag)
            return [(hidx, transform(hval)) for hidx, hval in result] if transform else result
