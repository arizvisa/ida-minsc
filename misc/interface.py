    ...
    def attach(self, target):
        ...
        def coroutine_when_enabled(hookq):
            ...
            try:
                ...
                while True:
                    ...
                    args, kwargs = parameters
                    logging.debug(u"{:s}.coroutine_when_enabled({!r}) : Coroutine for the target {:s} has received parameters ({:s}) and is still waiting for termination.".format('.'.join([__name__, self.__class__.__name__]), target, self.__formatter__(target), format_parameters(*args, **kwargs)))
                    ...
                    if not same_parameters((old_args, old_kwargs), parameters):
                        logging.info(u"{:s}.coroutine_when_enabled({!r}) : Parameters received for the target {:s} have unexpectedly changed from ({:s}) to ({:s}) while waiting for termination.".format('.'.join([__name__, self.__class__.__name__]), target, self.__formatter__(target), parameters_description, format_parameters(*args, **kwargs)))
                    continue
        ...
        def closure_start(*args, **kwargs):
            ...
            if not isinstance(result, Signal):
                logging.critical(u"{:s}.closure_start({!r}) : Coroutine #{:d} for the {:s}target {:s} returned a non-signal ({!r}) which will be returned explicitly.".format('.'.join([__name__, self.__class__.__name__]), target, len(State.running_queue), 'disabled ' if target in self.__disabled else '', self.__formatter__(target), result))
                ok = False
        ...
        def closure_cancel(*args, **kwargs):
            ...
            if result != State.END:
                ...
                logging.info(u"{:s}.closure_cancel({!r}) : Coroutine #{:d} for the {:s}target {:s} has discarded the first {:s} due to being canceled.".format('.'.join([__name__, self.__class__.__name__]), target, len(State.running_queue), 'disabled ' if target in self.__disabled else '', self.__formatter__(target), "ignored result ({!r})".format(result) if isinstance(result, (Signal, internal.types.none)) else "result ({!r})".format(result)))
                while result != State.END:
                    result = coro.send(parameters)
                    logging.info(u"{:s}.closure_cancel({!r}) : Coroutine #{:d} for the {:s}target {:s} has discarded the next {:s} due to being canceled.".format('.'.join([__name__, self.__class__.__name__]), target, len(State.running_queue), 'disabled ' if target in self.__disabled else '', self.__formatter__(target), "ignored result ({!r})".format(result) if isinstance(result, (Signal, internal.types.none)) else "result ({!r})".format(result)))
        ...
        def closure_stop(*args, **kwargs):
            ...
            if result != State.END:
                ...
                logging.info(u"{:s}.closure_stop({!r}) : Coroutine #{:d} for the {:s}target {:s} has discarded the first {:s} due to requested stop.".format('.'.join([__name__, self.__class__.__name__]), target, len(State.running_queue), 'disabled ' if target in self.__disabled else '', self.__formatter__(target), "ignored result ({!r})".format(result) if isinstance(result, (Signal, internal.types.none)) else "result ({!r})".format(result)))
                while result != State.END:
                    result = coro.send(parameters)
                    logging.info(u"{:s}.closure_stop({!r}) : Coroutine #{:d} for the {:s}target {:s} has discarded the next {:s} due to requested stop.".format('.'.join([__name__, self.__class__.__name__]), target, len(State.running_queue), 'disabled ' if target in self.__disabled else '', self.__formatter__(target), "ignored result ({!r})".format(result) if isinstance(result, (Signal, internal.types.none)) else "result ({!r})".format(result)))
        ...
        def closure_resume(*args, **kwargs):
            ...
            if not State.running_queue:
                logging.info(u"{:s}.closure_resume({!r}) : No coroutines for the {:s}target {:s} exist within the running queue and will require manual start".format('.'.join([__name__, self.__class__.__name__]), target, 'disabled ' if target in self.__disabled else '', self.__formatter__(target)))
            ...
            if not is_coroutine_started(coro):
                logging.critical(u"{:s}.closure_resume({!r}) : Coroutine #{:d} for the {:s}target {:s} was unable to be resumed due to a start failure.".format('.'.join([__name__, self.__class__.__name__]), target, len(State.running_queue), 'disabled ' if target in self.__disabled else '', self.__formatter__(target)))
                return
...
class priorityhook(prioritybase):
        ...
        if not ok:
            for name in sorted(self.__attached__):
                if name in self.__attached_scope:
                    ...
                    logging.debug(u"{:s}.close() : Detaching target {:s} still resulted in the start instance ({!s}) and stop instance ({!s}) still being attached to target {:s}.".format('.'.join([__name__, cls.__name__]), self.__formatter__(target), start, stop, self.__formatter__(target)))

                elif self.__attached_instances.get(name, []):
                    count = len(self.__attached_instances.get(name, []))
                    logging.debug(u"{:s}.close() : Detaching target {:s} left {:d} instance{:s} still being attached to target {:s}.".format('.'.join([__name__, cls.__name__]), self.__formatter__(target), count, '' if count == 1 else 's', self.__formatter__(target)))
    ...
    def __unhook_order(self, name):
        ...
        # First check that our target is attached. If it isn't, then nothing
        # is attached and technically we were successful in detaching from it.
        ...
        failures, instances = [], self.__attached_instances.get(name, [])
        for index, instance in enumerate(instances):
            if not instance.unhook():
                logging.warning(u"{:s}._unhook_order({!r}) : Unable to unhook instance ({!s}) {:d} of {:d} for the target {:s}.".format('.'.join([__name__, cls.__name__]), name, instance, 1 + index, len(instances), self.__formatter__(name)))
                failures.append(index)
            continue

        # If we couldn't unhook them, then we bail before doing anything serious.
        if failures:
            ...
            logging.critical(u"{:s}._unhook_order({!r}) : Unable to unhook {:d} instance{:s} ({:s}) that {:s} currently managing the target {:s}.".format('.'.join([__name__, cls.__name__]), name, len(failures), '' if len(failures) == 1 else 's', description, 'is' if len(failures) == 1 else 'are', self.__formatter__(name)))
            return False

        # Now we can go through and unhook the instances that manage the scope.
        start, stop = self.__attached_scope[name]

        start_removed = start.unhook()
        if not start_removed:
            logging.warning(u"{:s}._unhook_order({!r}) : Unable to unhook the start instance ({!s}) for the target {:s}.".format('.'.join([__name__, cls.__name__]), name, start, self.__formatter__(name)))

        stop_removed = stop.unhook()
        if not stop_removed:
            logging.warning(u"{:s}._unhook_order({!r}) : Unable to unhook the stop instance ({!s}) for the target {:s}.".format('.'.join([__name__, cls.__name__]), name, stop, self.__formatter__(name)))

        # If both were successful, then we did good and can exit cleanly.
        if start_removed and stop_removed:
            return True
        ...
        if all([not start_removed, not stop_removed]):
            logging.critical(u"{:s}._unhook_order({!r}) : Unable to unhook the scope hooks ({:s} and {:s}) for the target {:s} which will result in hooks being unreliable.".format('.'.join([__name__, cls.__name__]), name, start, stop, self.__formatter__(name)))

        elif any([not stop_removed, not start_removed]):
            description = "{:s} callable ({!s})".format('stop', stop) if not stop_removed else "{:s} callable ({!s})".format('start', start) if not start_removed else "start ({!s}) and stop ({!s}) callables".format(start, stop)
            logging.warning(u"{:s}._unhook_order({!r}) : Unable to unhook the {:s} from target {:s} which will result in the appearance of the target being disabled.".format('.'.join([__name__, cls.__name__]), name, description, self.__formatter__(name)))

        return False

    def __hook_order(self, name):
        '''Iterate through the known instances for the target `name` and enable their hooks in the correct order.'''
        cls = self.__class__

        # First check that our target is attached. If it isn't, then we
        # really can't do anything as there's no attached hooks to disable.
        if name not in self.__attached__:
            return False
    ...
    def attach(self, name):
        ...
        logging.warning(u"{:s}.attach({!r}) : Unexpected error trying to rehook the assigned hooks after attaching to target {:s}.".format('.'.join([__name__, cls.__name__]), name, self.__formatter__(name)))
        return False
...
class priorityhxevent(prioritybase):
    ...
    def attach(self, event):
        ...
        if event not in self.__attached__:
            ...
            if not module.install_hexrays_callback(closure_stop):
            ...
            if module.install_hexrays_callback(closure_start):
            ...
            expected, res = 1, module.remove_hexrays_callback(closure_stop)
            if res:
        ...
        res = module.remove_hexrays_callback(closure_start)
        ...
        if current < count:
            while self.__attached_count[event] < count:
                res = module.install_hexrays_callback(closure_resume)
                if not res:
                    break
            ...
            while self.__attached_count[event] > count:
                res = module.remove_hexrays_callback(closure_resume)
                if not res:
                    break
                self.__attached_count[event] -= res
        ...
        elif module.install_hexrays_callback(closure_start):
    ...
    def close(self):
        ...
        if not super(priorityhxevent, self).close():
            ...
            [logging.debug(u"{:s}.close() : Event {:s} is still attached{:s}.".format('.'.join([__name__, cls.__name__]), self.__formatter__(event), " by {:d} callback{:s}".format(self.__attached_count[event], '' if self.__attached_count[event] == 1 else 's') if event in self.__attached__ else '')) for event in self]
    ...
    def __remove_callbacks(self, event):
        ...
        # If the decompiler hasn't loaded, then we can't do shit.
        elif not getattr(self, '__hexrays_ready__', False):
            raise internal.exceptions.UnsupportedCapability(u"{:s}.__remove_callbacks({!r}) : Unable to detach from the event {:s} due to the decompiler being unavailable.".format('.'.join([__name__, cls.__name__]), event, self.__formatter__(event)))

        module = self.__hexrays_module__

        # Grab the callables that were previously attached.
        closure_start, closure_resume, closure_stop = self.__attached__[event]
        ...
        while self.__attached_count[event] > 0:
            index, res = needed - self.__attached_count[event], module.remove_hexrays_callback(closure_resume)
        ...
        expected, start_count = 1, module.remove_hexrays_callback(closure_start)
        if start_count:
        ...
        expected, stop_count = 1, module.remove_hexrays_callback(closure_stop)
        if stop_count:
    ...
    def left(cls, offset, items):
        ...
        for offset, item in cls.layout(beginning, layout, +1):
            if isinstance(item, (internal.structure.structure_t, idaapi.struc_t, internal.structure.members_t)):
                ...
                mptr, fullname, sptr = idaapi.get_member_by_id(item.id)
                result.append(internal.structure.new(sptr.id, -item.realoffset + offset).members[item.index])
...
class function(object):
    ...
    def chunk(cls, func, ea):
        ...
        for chunk in cls.chunks(func):
            left, right = range.bounds(chunk)
            if left <= ea < right:
                area = bounds_t(left, right)
                return area.range()
            continue
        return

    @classmethod
    def frame_offset(cls, func):
        ...
        return 0 if fn.frame == idaapi.BADNODE else -idaapi.frame_off_args(fn)

    @classmethod
    def frame_disassembler_offset(cls, func):
        '''Return the disassembler offset used by the frame belonging to the function `func`.'''
        fn = func if isinstance(func, idaapi.func_t) else cls.by(func)
        return 0 if fn.frame == idaapi.BADNODE else -fn.frsize
    ...
    def frame(cls, func):
        ...
        if fn.frame == idaapi.BADNODE or not sptr:
            raise E.MissingTypeOrAttribute(u"{:s}.frame({:#x}) : The specified function does not have a frame.".format('.'.join([__name__, cls.__name__]), range.start(fn)))
    ...
    def color(cls, func, *rgb):
        ...
        if rgb:
            r, b = (operator.and_(0xff * shift, *rgb) // shift for shift in [0x010000, 0x000001])
            func.color = DEFCOLOR if operator.contains({None, DEFCOLOR}, *rgb) else sum([b * 0x010000, operator.and_(0x00ff00, *rgb), r * 0x000001])
    ...
    def blockcolor(cls, bb, *rgb, **frame):
        ...
        DEFCOLOR, framecolorQ = 0xffffffff, operator.contains(frame, 'frame') and (not isinstance(frame['frame'], internal.types.bool) or (rgb and frame['frame']))

        # Grab the node information and the color from its attributes.
        fn, ni = function.by(range.start(bb)), idaapi.node_info_t()
        ...
        if rgb and framecolorQ and isinstance(framergb, internal.types.integer):
            [rgb], original, flags = rgb, ni.bg_color, idaapi.NIF_BG_COLOR | idaapi.NIF_FRAME_COLOR

            # Assign the necessary attributes to apply the chosen colors.
            r, b = (operator.and_(0xff * shift, rgb) // shift for shift in [0x010000, 0x000001])
            ni.bg_color = sum([b * 0x010000, 0x00ff00 & rgb, r * 0x000001])
            r, b = (operator.and_(0xff * shift, framergb) // shift for shift in [0x010000, 0x000001])
            ni.frame_color = sum([b * 0x010000, 0x00ff00 & framergb, r * 0x000001])
            clr_node_info(ea, bb.id, flags) if (rgb, framergb) == (DEFCOLOR, DEFCOLOR) else set_node_info(ea, bb.id, ni, flags)

        # Set the frame color (RGB) of the basic block at `bb` to `rgb`.
        elif framecolorQ:
            rgb, original, flags = framergb, ni.frame_color, idaapi.NIF_FRAME_COLOR
            r, b = (operator.and_(0xff * shift, rgb) // shift for shift in [0x010000, 0x000001])
            ni.frame_color = sum([b * 0x010000, 0x00ff00 & rgb, r * 0x000001])
            clr_node_info(ea, bb.id, flags) if rgb == DEFCOLOR else set_node_info(ea, bb.id, ni, flags)

        # Set the background color (RGB) of the basic block at `bb` to `rgb`.
        elif rgb and not framecolorQ:
            [rgb], original, flags = rgb, ni.bg_color, idaapi.NIF_BG_COLOR
            r, b = (operator.and_(0xff * shift, rgb) // shift for shift in [0x010000, 0x000001])
            ni.bg_color = sum([b * 0x010000, 0x00ff00 & rgb, r * 0x000001])
            clr_node_info(ea, bb.id, flags) if rgb == DEFCOLOR else set_node_info(ea, bb.id, ni, flags)
...
class name(object):
    ...
    def mangled(cls, ea, string):
        ...
        return {idaapi.MANGLED_CODE: idaapi.FF_CODE, idaapi.MANGLED_DATA: idaapi.FF_DATA}.get(mangled_t, idaapi.FF_UNK)
