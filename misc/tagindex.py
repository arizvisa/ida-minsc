...
class schema(object):
    ...
    def version(cls):
        ...
        if netnode.sup.has(node, NSUP_SCHEMA_VERSION, tag=cls.statstag):
            return netnode.sup.get(node, NSUP_SCHEMA_VERSION, types.integer, tag=cls.statstag)
        return
    ...
    def iterate(cls, *args, **kwargs):
        '''Iterate through each key and usage mask for the current schema.'''
        return cls.forward(*args, **kwargs)

    @classmethod
    @abc.abstractmethod
    def forward(cls, node, *key, **tag):
        '''Yield the key and usage mask in order for the current schema from netnode `node` starting at `key` (if given).'''
    ...
    def backward(cls, node, *key, **tag):
        '''Yield the key and usage mask in reverse for the current schema from netnode `node` starting at `key` (if given).'''
...
class tags(schema):
    ...
    def get(cls, name):
        ...
        position = netnode.hash.get(node, tagname, types.integer, tag=cls.nametag)
        if netnode.sup.has(node, position, tag=cls.counttag):
            return position, netnode.sup.get(node, position, types.integer, tag=cls.counttag)
        return position, 0
    ...
    def add(cls, name, *count):
        ...
        ok_count = netnode.sup.set(node, position, count, tag=cls.counttag)
    ...
    def discard(cls, name):
        ...
        count = netnode.sup.get(node, position, types.integer, tag=cls.counttag)
        if not netnode.sup.remove(node, position, tag=cls.counttag):
    ...
    def count(cls, name):
        ...
        if netnode.sup.has(node, position, tag=cls.counttag):
            return netnode.sup.get(node, position, types.integer, tag=cls.counttag)
        return 0
    ...
    def counts(cls):
        ...
        for name, position in cls.iterate():
            if not netnode.sup.has(node, position, tag=cls.counttag):
                continue
            count = netnode.sup.get(node, position, types.integer, tag=cls.counttag)
            yield name, count
        return
    ...
    def increment(cls, name, amount=1):
        ...
        available, count = netnode.sup.has(node, position, tag=cls.counttag), 0
        if available:
            count = netnode.sup.get(node, position, types.integer, tag=cls.counttag)
        ...
        # previous value.
        if not netnode.sup.set(node, position, count + amount, tag=cls.counttag):
    ...
    def decrement(cls, name, amount=1):
        ...
        elif not netnode.sup.set(node, position, count - amount, tag=cls.counttag):
    ...
    def guarded_increment(cls, name, amount=1):
        ...
        try:
            ...
            if not netnode.sup.set(cls.node(), position, count, tag=cls.counttag):
    ...
    def transactional_adjust(cls, name):
        ...
        if isinstance(name, types.string) and netnode.hash.has(node, tagname, tag=cls.nametag):
            position = netnode.hash.get(node, tagname, types.integer, tag=cls.nametag)
            available = netnode.sup.has(node, position, tag=cls.counttag)
            count = netnode.sup.get(node, position, types.integer, tag=cls.counttag) if available else 0
        ...
        def adjustment_closure(position, count):
            ...
            try:
                while True:
                    ...
                    # should be raising an exception since it doesn't exist.
                        ...
                        raise OverflowError(u"{:s}.transactional_adjust({!s}) : Unable to adjust the reference count by {:+d} for the tag \"{!s}\" due to the result being smaller than the minimum value ({:#x}{:+#x}).".format('.'.join([__name__, cls.__name__]), "{!r}".format(name), amount, utils.string.escape(tagname, '"'), count, adjustment))
            ...
            # if the generator is done, then calculate the new reference count.
                ...
                E = OverflowError(u"{:s}.transactional_adjust({!s}) : Aborting the transaction to set the reference count for tag \"{:s}\" ({:#x}) to an out-of-bounds value {:#x}.".format('.'.join([__name__, cls.__name__]), "{!r}".format(name), utils.string.escape(tagname, '"'), count, newcount))
                assert(0 <= newcount < MAXIMUM_SPEC_INTEGER), E

            # if an exception was raised, log an error and discard our transaction.
            except:
                logging.info(u"{:s}.transactional_adjust({!s}) : Aborting the transaction to set the reference count for tag \"{:s}\" ({:d}) from its previous value ({:d}) to the new value {:d} ({:+d}).".format('.'.join([__name__, cls.__name__]), "{!r}".format(name), utils.string.escape(name, '"'), position, count, count + adjustment, adjustment))
                raise
            ...
            elif not netnode.sup.set(node, position, newcount, tag=cls.counttag):
        ...
        # if the count was 0, then we remove the tag since we had to add it.
            ...
            ok_name = netnode.hash.remove(node, tagname, tag=cls.nametag)
            ok_index = netnode.sup.remove(node, position, tag=cls.indextag)
            ok_count = netnode.sup.remove(node, position, tag=cls.counttag)
...
class counted(schema):
    ...
    namespace to store an explicit mask, but also expose a usage mask to
    ...
    is a "supval" table that stores big integers for the usage mask. The second
    ...
    usage mask for the tags applied to a key by removing it if the reference
    count is 0.
    ...
    When the tag being incremented does not exist in the usage mask for the
    chosen key, the reference count for the tag is created and the bit
    position for the tag is set in its corresponding usage mask. If decrementing
    the tag results in the reference count being 0, the bit position for the tag
    is removed from the usage mask associated with the given key.
    """
    ...
    def hascount(cls, node, key, position, tag):
        ...
        return netnode.sup.has(node, position, tag=tag)
    ...
    def getcount(cls, node, key, position, tag):
        ...
        if netnode.sup.has(node, position, tag=tag):
            return netnode.sup.get(node, position, types.integer, tag=tag)
        return 0
    ...
    def setcount(cls, node, key, position, count, tag):
        ...
        if count > 0:
            return netnode.sup.set(node, position, count, tag=tag)
        return netnode.sup.remove(node, position, tag=tag)
    ...
    def decrement(cls, node, key, name, tag):
        ...
        with tags.guarded_decrement(name) as (position, tagcount):
            ...
            # start by removing the tag from the usage mask for the key.
            ...
            # if the usage mask already had its values cleared, then there is an
            # index discrepancy. we leave successfully, but log a complaint.
            elif not(used & bit):
                logging.warning(u"{:s}.decrement({:#x}, {:#x}, {!s}, {:#x}) : Ignoring inconsistency with the usage mask at {:#x} for the specified tag ({!s}).".format('.'.join([__name__, cls.__name__]), node, key, "{!r}".format(name), tag, key, "{!r}".format(name)))
                return position, count - 1
            ...
            # and restore the usage mask too if it also had been updated.
            bits = used.bit_count() if hasattr(used, 'bit_count') else "{:b}".format(used).count('1')
            logging.info(u"{:s}.decrement({:#x}, {:#x}, {!s}, {:#x}) : Rolling back the usage mask for {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), node, key, "{!r}".format(name), tag, key, bits))
            if used & bit and not cls.setusage(node, key, used & clear, tag):
                logging.error(u"{:s}.decrement({:#x}, {:#x}, {!s}, {:#x}) : Unable to roll back the usage mask for {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), node, key, "{!r}".format(name), tag, key, bits))
...
class globals(counted):
    ...
    def forward(cls, *ea):
        ...
        node = cls.node()
        for key, integer in super(globals, cls).forward(node, *ea, tag=cls.addresstag):
    ...
    def backward(cls, *ea):
        ...
        node = cls.node()
        for key, integer in super(globals, cls).backward(node, *ea, tag=cls.addresstag):
            yield idaapi.node2ea(key), integer
        return
    ...
    def range(cls, start, stop):
        ...
        node = cls.node()
        items = suptools.range(node, start, stop, tag=cls.addresstag)
        return [(idaapi.node2ea(ea), integer) for ea, integer in items]
    ...
    def select(cls, addresses):
        ...
        for ea in map(idaapi.ea2node, iterable):
            mask = suptools.bigint(node, idaapi.ea2node(ea), tag=cls.addresstag)
            yield ea, mask
        return
    ...
    def repr(cls, *pattern):
        ...
        items = [(ea, integer) for ea, integer in cls.iterate()]

        iterable = (','.join(map("{:d}".format, tags.explode(integer))) for ea, integer in items if Fmatch("{:#x}".format(ea)))
...
class contents(counted):
    ...
    # FIXME: track the addresses associated with a contents address. this is
    #        because we need a way of knowing which addresses are empty and
    #        don't belong to the function anymore.
    ownershiptag = netnode.alttag + 1
    ...
    # FIXME: an address can have multiple owners, so we'll need to ensure all
    #        functions owning said address are updated. this will likely require
    #        a custom implementation of increment and decrement.
    ...
    def adjust_owners(cls, owners, key, name, adjustment):
        ...
        with tags.transactional_adjust(name) as (position, tagcount):
            ...
            # get the reference count and the usage masks for the given key in
            ...
            usage = {owner : suptools.bigint(usagenode, owner, tag=cls.usagetag) for owner in counts}
            ...
            setting = {owner : used | bit for owner, used in usage.items() if not(used & bit)}
            clearing = {owner : used & clear for owner, used in usage.items() if used & bit}
            newused = clearing if adjustment < 0 else setting if adjustment > 0 else {}
            ...
            try:
                ...
                for owner, newcount in newcounts.items():
                    ...
                    ok_mask = owner not in newused or suptools.setbigint(usagenode, owner, newused[owner], tag=cls.usagetag)
                ...
                if failed:
                    raise exceptions.DisassemblerError(u"{:s}.adjust_owners({:s}, {:#x}, {!s}, {:+d}) : Unable to update the key at {:#x} of the specified netnodes ({:s}) for the given tag ({!s}).".format('.'.join([__name__, cls.__name__]), owners_description, key, "{!r}".format(name), adjustment, key, ', '.join(owners_description), "{!r}".format(name)))
            ...
            # can send the exception to the coroutine to abort the transaction.
                ...
                for owner, count in counts.items():
                    ...
                    elif not suptools.setbigint(usagenode, owner, used, tag=cls.usagetag):
            ...
            written = {owner : suptools.bigint(usagenode, owner, tag=cls.usagetag) for owner, _ in newused.items()}
            ...
            elif not all(newcounts[owner] == count for owner, count in adjusted.items()):
    ...
    def increment(cls, ea, name):
        ...
        node, owners = cls.node(), interface.function.owners(ea)
        nodes = {owner : idaapi.ea2node(owner) for owner in owners}

        position, count = tags.get(name) if tags.has(name) else tags.add(name)
        bit, clear = pow(2, position), ~pow(2, position)

        # grab the mask for the given address and check that our tag isn't
        # inside it. if it is, then we don't need to do anything.
        res = suptools.bigint(node, idaapi.ea2node(ea), tag=cls.addresstag)
        if res & bit:
            return position, {owner : cls.getcount(node, idaapi.ea2node(ea), position, tag=cls.counttag) for owner, node in nodes.items()}

        # otherwise we can just update the mask with the bit for the new tag.
        elif not suptools.setbigint(node, idaapi.ea2node(ea), res | bit, tag=cls.addresstag):
            raise exceptions.DisassemblerError(u"{:s}.increment({:#x}, {!s}) : Unable to increment the reference count of the specified tag ({!s}) for contents address {:#x} in the current netnode ({:#x}).".format('.'.join([__name__, cls.__name__]), ea, "{!r}".format(name), "{!r}".format(name), ea, node))

        # grab all the potential function owners for the given address, and then
        # update the usagetag table for each owner with the newly-added tag.
        try:
            result = cls.adjust_owners(owners, ea, name, +1)

        # if an exception was raised, then we need to undo the usage mask
        # setting that we applied before calling our super method.
        except:
            bits = res.bit_count() if hasattr(res, 'bit_count') else "{:b}".format(res).count('1')
            logging.info(u"{:s}.increment({:#x}, {!s}) : Rolling back the usage mask for the contents address at {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), ea, "{!r}".format(name), ea, bits))
            if not suptools.setbigint(node, idaapi.ea2node(ea), res, tag=cls.addresstag):
                logging.error(u"{:s}.increment({:#x}, {!s}) : Unable to roll back the usage mask for the contents address at {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), ea, "{!r}".format(name), ea, bits))

            # FIXME: to properly roll-back the value, we will need to remove the
            #        tag if it was added at the beginning of this function.
            #        we're not doing it since tags.discard is kinda complex.
            raise
        return result
    ...
    def decrement(cls, ea, name):
        ...
        # grab the bit position, and use it to calculate some integers that we
        # can use for setting and clearing the tags at the given address.
        position, count = tags.get(name)
        bit, clear = pow(2, position), ~pow(2, position)

        # next we'll check if the tag exists at the desired address. if not,
        # then there's nothing to do and we can just leave.
        res = suptools.bigint(node, idaapi.ea2node(ea), tag=cls.addresstag)
        if not(res & bit):
            return position, {owner : cls.getcount(node, idaapi.ea2node(ea), position, tag=cls.counttag) for owner, node in nodes.items()}

        # otherwise, we need to clear the bit position from the address.
        elif not suptools.setbigint(node, idaapi.ea2node(ea), res & clear, tag=cls.addresstag):
            raise exceptions.DisassemblerError(u"{:s}.decrement({:#x}, {!s}) : Unable to decrement the reference count of the specified tag ({!s}) for contents address {:#x} in the current netnode ({:#x}).".format('.'.join([__name__, cls.__name__]), ea, "{!r}".format(name), "{!r}".format(name), ea, node))

        # update the usagetag table with the tags that were removed.
        try:
            result = cls.adjust_owners(owners, ea, name, -1)

        except:
            bits = res.bit_count() if hasattr(res, 'bit_count') else "{:b}".format(res).count('1')
            logging.info(u"{:s}.decrement({:#x}, {!s}) : Rolling back the usage mask for the contents address at {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), ea, "{!r}".format(name), ea, bits))
            if not suptools.setbigint(node, idaapi.ea2node(ea), res, tag=cls.addresstag):
                logging.error(u"{:s}.decrement({:#x}, {!s}) : Unable to roll back the usage mask for the contents address at {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), ea, "{!r}".format(name), ea, bits))
            raise
        return result
    ...
    def forward(cls, *ea):
        ...
        node = cls.node()
        for key, integer in super(contents, cls).forward(node, *ea, tag=cls.addresstag):
            yield idaapi.node2ea(key), integer
        return
    ...
    def backward(cls, *ea):
        ...
        node = cls.node()
        for key, integer in super(contents, cls).backward(node, *ea, tag=cls.addresstag):
            yield idaapi.node2ea(key), integer
        return
    ...
    def range(cls, start, stop):
        ...
        node = cls.node()
        items = super(contents, cls).range(node, start, stop, tag=cls.addresstag)
        return [(idaapi.node2ea(ea), integer) for ea, integer in items]
    ...
    def erase(cls, func):
        ...
        fn = func if isinstance(func, types.integer) and netnode.sup.has(node, func, cls.usagetag) else interface.function.by(func)
        ...
        if not netnode.sup.remove(node, key, cls.usagetag):
            logging.error(u"{:s}.erase({:#x}) : Unable to remove the usage mask for the function at {:#x} from netnode {:#x}.".format('.'.join([__name__, cls.__name__]), parameter, "{!r}".format(name), fn, fn, node))

        for position in netnode.sup.fiter(key, cls.counttag):
            if netnode.sup.remove(key, position, tag=cls.counttag):
                continue
            logging.error(u"{:s}.erase({:#x}) : Unable to remove the reference count for the specified tag ({:d}) from netnode {:#x}.".format('.'.join([__name__, cls.__name__]), parameter, position, key))
        ...
        for ea, used in deleting.items():
            tags = tags.names(used)
            [cls.decrement(ea, name) for name in tags]
    ...
    def select(cls, *ea):
        ...
        node = cls.node()
        for key, integer in super(contents, cls).forward(node, *ea, tag=cls.usagetag):
...
class members(counted):
    ...
    def erase(cls, sid, mids):
        ...
        iterable = mids if isinstance(mids, types.unordered) else [mids]
...
class structure(schema):
    ...
    def increment(cls, sid, name):
        ...
        # if we fail, tho, we need to undo the mask we just updated for the tag.
            ...
            bits = res.bit_count() if hasattr(res, 'bit_count') else "{:b}".format(res).count('1')
            logging.info(u"{:s}.increment({:#x}, {!s}) : Rolling back the usage mask for structure {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), sid, "{!r}".format(name), sid, bits))
            if not suptools.setbigint(node, sid, res, tag=cls.typetag):
                logging.error(u"{:s}.increment({:#x}, {!s}) : Unable to roll back the usage mask for structure {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), sid, "{!r}".format(name), sid, bits))
            raise
        return result
    ...
    def decrement(cls, sid, name):
        ...
        # if doing that raised an exception, then we undo the usage mask that
        # was just applied to the structure id that we were given.
        except:
            bits = res.bit_count() if hasattr(res, 'bit_count') else "{:b}".format(res).count('1')
            logging.info(u"{:s}.decrement({:#x}, {!s}) : Rolling back the usage mask for structure {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), sid, "{!r}".format(name), sid, bits))
            if not suptools.setbigint(node, sid, res, tag=cls.typetag):
                logging.error(u"{:s}.decrement({:#x}, {!s}) : Unable to roll back the usage mask for structure {:#x} to its previous number of bits ({:d}).".format('.'.join([__name__, cls.__name__]), sid, "{!r}".format(name), sid, bits))
            raise
        return result
    ...
    def erase(cls, structures):
        ...
        selected = {sid : used for sid, used in cls.select(structures)}
        for sid, used in selected.items():
            positions = tags.explode(used)
            [cls.decrement(sid, position) for position in positions]
        return sorted(selected)
