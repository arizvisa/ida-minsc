...
class ask(object):
    ...
    @document.parameters(default='if ``yes``, ``no``, or ``cancel`` is set to true, then use that value as the default')
    ...
    @internal.utils.multicase()
    @classmethod
    @document.parameters(default='the default address for the user to change. if one was not specified, then use the current address.')
    ...
    @internal.utils.multicase()
    @document.parameters(default='the default integer for the user to change. if one was not specified, then use the previous integer that the user input.')
    @classmethod
    ...
    @internal.utils.multicase()
    @document.parameters(default='the default segment for the user to change. if one was not specified, then use the address of the current segment.')
    @classmethod
    ...
    @internal.utils.multicase()
    @document.parameters(default='the default string for the user to change')
    @classmethod
...
class current(object):
    ...
    @document.aliases('current.opnum')
    @classmethod
    def operand(cls):
...
class message(object):
    ...
    """
    @document.parameters(message='the message to display in the dialog box', icon='a boolean specifying the icon to include in the dialog which can be either ``information`` or ``warning``')
    ...
    @document.aliases('message.info')
    @classmethod
    @document.parameters(message='the message to display in the dialog box')
    def information(cls, message):
        '''Display a modal information dialog box using the provided `message`.'''
        return cls(message, information=True)
    info = internal.utils.alias(information, 'message')

    @document.aliases('message.warn')
    @classmethod
    @document.parameters(message='the message to display in the dialog box')
...
class window(object):
    ...
    @document.parameters(xy='a tuple composed of the X and Y coordinates of a window')
    def at(cls, xy):
...
class widget(object):
    ...
    @document.parameters(title='a string containing the widget title')
    ...
    @classmethod
    @document.parameters(widget='the widget to open', flags='the specified flags to use when opening the widget')
    def open(cls, widget, flags, **target):
        ...
        return ok
    @classmethod
    @document.parameters(widget='the widget to close', flags='the specified flags to use when closing the widget')
    ...
    @classmethod
    @document.parameters(widget='the widget to show')
    def show(cls, widget):
    ...
    @classmethod
    @document.parameters(widget='the widget to put into focus')
    ...
    @classmethod
    @document.parameters(widget='the widget to return the type of')
    def type(cls, widget):
        '''Return the ``idaapi.twidget_type_t`` for the given `widget`.'''
        twidget = cls.form(widget) if cls.isinstance(widget) else widget
        return idaapi.get_widget_type(twidget)
    @classmethod
    @document.parameters(widget='the widget to return the title from')
    ...
    @document.parameters(title='the title of the desired widget to return')
    def by(cls, title):
    ...
    @classmethod
    @document.parameters(form='the IDA plugin form to convert to a user-interface widget')
    def of(cls, form):
        ...
        raise internal.exceptions.MissingMethodError
    @classmethod
    @document.parameters(widget='the user-interface widget to convert to an IDA plugin form')
    def form(cls, widget):
        ...
        raise internal.exceptions.MissingMethodError
    @classmethod
    @document.parameters(object='the object to check the instance of')
...
try:
    ...
    class application(application):
        ...
        @classmethod
        @document.parameters(x='the x-coordinate of the desired window', y='the y-coordinate of the desired window')
        def window(cls, x, y):
