...
def function(ea):
    '''Will serialize all the tags in a function to a string'''
    ea = fn.top(ea)
    data = fn.fetch(ea)
    return pickle.dumps(data)    

def database():
    '''Will serialize all the tags in all functions to a string'''
    result = dict( ((ea - db.baseaddress(),function(ea)) for ea in db.functions()) )
    return pickle.dumps(result)

# TODO: will need to dump the tags for stack variables and the rest of the function context
#       too
