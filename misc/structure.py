...
class members_t(object):
    ...
    def by(self, location):
        ...
        if (offset, size) != (member.offset, member.size):
            ...
            raise E.MemberNotFoundError(u"{:s}({:#x}).members.by({!s}) : The member ({:s}) at the given location ({:#x}<->{:#x}) {:s}.".format('.'.join([__name__, cls.__name__]), self.owner.id, location, member.name, member.left, member.right, message))
        return member
    ...
    def has(self, structure):
        ...
        for sptr, midx, mptr in members.iterate(self.owner.id):
            opinfo = idaapi.opinfo_t()
    ...
    def index(self, member):
        ...
        if not hasattr(member, 'id'):
            raise E.InvalidParameterError(u"{:s}({:#x}).members.index({!r}) : An invalid type ({!s}) was specified as the member being requested.".format('.'.join([__name__, cls.__name__]), owner.id, member, member.__class__))
    ...
    __members_matcher.combinator('tagged', utils.fcompose(utils.fcompose, utils.fcondition(utils.finstance(internal.types.bool, internal.types.integer), utils.finstance(internal.types.string))(utils.fcondition(operator.truth)(utils.fcompose(utils.fdiscard(internal.tags.select.members), utils.fpartial(utils.imap, utils.nth(0)), utils.fpartial(utils.imap, operator.attrgetter('id')), internal.types.set, utils.fpartial(utils.fpartial, operator.contains)), utils.fcompose(utils.fdiscard(internal.tags.select.members), utils.fpartial(utils.imap, utils.nth(0)), internal.types.set, utils.fpartial(utils.fpartial, utils.fcompose(operator.contains, operator.not_)))), utils.fcompose(internal.tags.select.members, utils.fpartial(utils.itermap, utils.nth(0)), utils.fpartial(utils.imap, operator.attrgetter('id')), internal.types.set, utils.fpartial(utils.fpartial, operator.contains)), utils.fcompose(internal.types.set, internal.tags.select.members, utils.fpartial(utils.itermap, utils.nth(0)), utils.fpartial(utils.imap, operator.attrgetter('id')), internal.types.set, utils.fpartial(utils.fpartial, operator.contains)))), 'id')
    ...
    def __getitem__(self, index):
        ...
        if res is None:
            ...
            raise E.MemberNotFoundError(u"{:s}({:#x}).members.__getitem__({:d}) : Unable to find a member {:s}.".format('.'.join([__name__, cls.__name__]), owner.id, index, where))
        return res
