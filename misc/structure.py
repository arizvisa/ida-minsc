...
def by_index(index):
    ...
    return by_index_or_identifier(index)
...
def union(type):
    ...
    if isinstance(type, structure_t):
        return union(type.ptr)
    elif isinstance(type, idaapi.tinfo_t):
        return type.is_union()
    elif hasattr(idaapi, 'get_struc') and isinstance(type, types.integer):
    ...
    elif isinstance(type, types.integer) and ti.get_type_by_tid(type):
        return ti.is_union()
...
class v9members(object):
    ...
    In this namespace, the first `v9members.by` function is an internal function
    that is used to get information about the structure or union member that is
    described by the parameters. It allows the caller to pass a combination of
    parameters in order to select the desired structure or union member.
    ...
    def index_before(cls, type, offset):
        ...
        moffset, msize = udm.offset, udm.size
        return mindex if offset <= moffset + msize else min(count, mindex + 1)
    ...
    def add(cls, type, name, typeinfo, location, *offset):
        ...
        ti, til = idaapi.tinfo_t(), interface.tinfo.library(typeinfo)
        if isinstance(type, idaapi.tinfo_t):
            ti, sid = interface.tinfo.copy(type), interface.tinfo.identifier(type)
        elif isinstance(type, types.integer) and interface.node.identifier(type) and ti.get_type_by_tid(type):
            ti, sid = ti, type
        elif isinstance(type, structure_t):
            ti, sid = address.type(type.id), type.id
        elif hasattr(idaapi, 'struc_t') and isinstance(type, idaapi.struc_t):
            ti, sid = address.type(type.id), type.id
        else:
            raise E.InvalidParameterError(u"{:s}.add({!s}, {!s}, {!r}, {!s}{!s}) : Unable to locate the type using an unsupported parameter type ({!s}).".format('.'.join([__name__, cls.__name__]), type, "{!r}".format(name) if isinstance(name, types.string) else name, "{!s}".format(typeinfo), location, "{:d}".format(*offset) if offset else '', type.__class__))

        # before doing anything, we need to know the base offset to make sense
        # of the location. so, we grab the function and unpack its base. we also
        ...
        # the type of location we were given. this is because our type can be a
        # structure, a frame, or a union.
        count = cls.count(type)
        index_or_offset = useroffset = int(location)
        userindex = realoffset = index_or_offset if is_union else index_or_offset // 8
        realindex = userindex if userindex <= count else count
        ...
        # now we're ready to normalize the name since we support packed names
        # with a numeric suffix.
        packedname = interface.tuplename(*name) if isinstance(name, types.ordered) else name or ''
        defaultname = v9member.default_name(ti, None, realindex if is_union else realoffset)
        suffix = realindex if is_union else interface.function.frame_member_offset(fn, realoffset) if is_frame else realoffset

        # then we need to verify that the name is valid in order to determine if
        # we need to add a suffix or use a default name we weren't given one.
        oldname = candidatename = packedname or defaultname
        while cls.has_name(ti, candidatename):
        ...
        location_description = "index {:d}".format(userindex) if is_union else "offset {:+#x}".format(useroffset)
        ...
        # if we're using a later disassembler version with the `tinfo_t.add_udm`
        # method, then we can use it to add the member. we start out by setting
        # each of the properties specified by the caller on the member type.
        if hasattr(ti, 'add_udm'):
            udm = udm_t()
            udm.offset = 0 if is_union else useroffset
            udm.size = 8 * interface.tinfo.size(mtype)
            udm.name = utils.string.to(newname)
            if not udm.type.deserialize(til, *mtype.serialize()):
                raise E.DisassemblerError(u"{:s}.add({:#x}, {!s}, {!s}, {!s}{!s}) : Unable to copy the specified type ({!r}) into the member being added.".format('.'.join([__name__, cls.__name__]), sid, name_description, typeinfo_description, location, offset_description, "{!s}".format(mtype)))
            udm.cmt = utils.string.to('')
            udm.effalign, udm.tafld_bits, udm.fda = 0, 0, 0

            # assign the correct parameters depending on whether the type is a
            # union or not, and then use the disassembler to add the member.
            parameters = [0, 1, location] if is_union else [0]
            ok = ti.add_udm(udm, *parameters)
            if ok != idaapi.TERR_OK:
                errname, errdesc = interface.tinfo.format_type_error(ok)
                description = "{:s} ({:s})".format(errname, errdesc) if errname and errdesc else errname if errname else "({:d})".format(terr)
                raise E.DisassemblerError(u"{:s}.add({:#x}, {!s}, {!s}, {!s}{!s}) : Unable to add member \"{:s}\" at {:s} of the {:s} due to error {!s}.".format('.'.join([__name__, cls.__name__]), sid, name_description, typeinfo_description, location, offset_description, utils.string.escape(newname, '"'), location_description, type_description, description))
            ...
            key = udm_t()
            key.name = utils.string.to(newname)
            mindex = utd.find_member(key, idaapi.STRMEM_NAME)
            if not(0 <= mindex < utd.size()):
                raise E.MemberNotFoundError(u"{:s}.add({:#x}, {!s}, {!s}, {!s}{!s}) : Unable to locate a member with the specified name \"{:s}\" for the given {:s} ({:#x}).".format('.'.join([__name__, cls.__name__]), sid, name_description, typeinfo_description, location, offset_description, utils.string.escape(newname, '"'), type_description, sid))
            ...
            if mindex != ti.find_udm(udm, idaapi.STRMEM_INDEX):
                raise E.MemberNotFoundError(u"{:s}.add({:#x}, {!s}, {!s}, {!s}{!s}) : Unable to find the member at index {:d} of the specified {!s} ({:#x}).".format('.'.join([__name__, cls.__name__]), sid, name_description, typeinfo_description, location, offset_description, mindex, type_description, sid))
        ...
        # FIXME: is there a way to use the `idaapi.udt_type_data_t.insert`
        #        method to insert a member without having to copy all of them?
        if is_union:
            newutd = idaapi.udt_type_data_t()
            newutd.resize(1 + utd.size())
            for mindex in builtins.range(location):
                newutd[mindex].offset = utd[mindex].offset
                ...
                if not newutd[mindex].type.deserialize(til, *utd[mindex].type.serialize()):
                    raise E.DisassemblerError(u"{:s}.add({:#x}, {!s}, {!s}, {!s}{!s}) : Unable to copy the specified type ({!r}) at index {:d} for a member being preserved.".format('.'.join([__name__, cls.__name__]), sid, name_description, typeinfo_description, location, offset_description, "{!s}".format(mtype), mindex))
            ...
            newutd[location].name = utils.string.to(newname)
            ...
            for mindex in builtins.range(location, utd.size()):
                newutd[1 + mindex].offset = utd[mindex].offset
                ...
                newutd[1 + mindex].fda = utd[mindex].effalign

            # now we just need to use the index to get the udm of the added
            # member so that we can return it to the caller.
            udm = udm_t()
            udm.offset = location
            if mindex != ti.find_udm(udm, idaapi.STRMEM_INDEX):
                raise E.MemberNotFoundError(u"{:s}.add({:#x}, {!s}, {!s}, {!s}{!s}) : Unable to find the member at index {:d} of the specified {!s} ({:#x}).".format('.'.join([__name__, cls.__name__]), sid, name_description, typeinfo_description, location, offset_description, mindex, type_description, sid))
        ...
        raise E.NotImplementedError(u"{:s}.add({:#x}, {!s}, {!s}, {!s}{!s}) : Adding a member to a type in earlier versions of the disassembler is currently unsupported.".format('.'.join([__name__, cls.__name__]), sid, name_description, typeinfo_description, location, offset_description))
    ...
    def has_bounds(cls, type, start, stop):
        ...
        key, is_union, is_variable = udm_t(), union(tinfo), tinfo.is_varstruct()
    ...
    def at_offset(cls, type, offset):
        ...
        is_union, is_variable = union(tinfo), tinfo.is_varstruct()
        ...
        elif is_variable and not(udm.size):
            if udm.offset <= realoffset:
        ...
        elif udm.offset <= realoffset < udm.offset + udm.size:
            yield tinfo, mindex, udm
        return
    ...
    def at(cls, type, offset, *filter):
        ...
        while filtered:
            ...
            base, offset, moffset, count = base, remainder, 0 if union(mowner) else udm.offset, count + 1
    ...
    def remove_slice(cls, type, slice, *offset):
        ...
        if isinstance(type, idaapi.tinfo_t):
            ...
            raise E.InvalidParameterError(u"{:s}.remove_slice({!s}, {!s}{:s}) : Unable to locate the type using an unsupported parameter type ({!s}).".format('.'.join([__name__, cls.__name__]), "{!r}".format(type), slice, ", {:+#x}".format(*offset) if offset else '', type.__class__))
        ...
        # Get the details for the structure type we were given. Once we got
        # that, we can gather its attributes and turn our `slice` into a slice.
        utd = idaapi.udt_type_data_t()
        if not ti.get_udt_details(utd):
            raise E.DisassemblerError(u"{:s}.remove_slice({:#x}, {!s}{:s}) : Unable to get the details for the specified {:s} type.".format('.'.join([__name__, cls.__name__]), sid, slice, offset_description, type_description))
        ...
        for mowner, mindex, udm in cls.iterate(ti, builtins.slice(lindex, None)):
            ...
            for ea, _, _ in interface.xref.to(mid, idaapi.XREF_ALL):
                member_references.setdefault(mid, []).append(ea)
            continue

        # Clear out all the references to any special members, and then invert
        # our references dictionary so that they are keyed by their address.
        ...
        # Last thing to do before we delete the members, is to use the slice to
        ...
        # Finally we can start asking the disassembler to delete each member
        # specified by our list of indices.
        count, failed = 0, {mbitoffset for mbitoffset in []}
        for mid, mname, mindex, mbitoffset, mbits in listable:
            mkey = mindex if is_union else mbitoffset - base
            if mkey not in members:
                continue
            bits = 8 * interface.tinfo.size(ti)

            # Get the index for the member, and then we can just delete it.
            ...
            # If we succeeded, but there's still a member at that offset..then
            # something unexpected happened since it should've been removed.
            elif not is_union and v9members.has_offset(ti, mbitoffset - base) and not(next(cls.at_offset(ti, mbitoffset - base))[-1].is_gap()):
                logging.warning(u"{:s}.remove_slice({:#x}, {!s}{:s}) : Member \"{:s}\" ({:#x}) at {:s} of {:s} was not removed.".format('.'.join([__name__, cls.__name__]), sid, slice, offset_description, utils.string.escape(mname, '"'), mid, location_description, type_description))
                ok, _ = idaapi.TERR_OK, failed.add((mid, mbitoffset))
            ...
            # If the type was already reduced in size due to the removal of the
            # selected member, then we don't have to do anything with it.
            elif 8 * interface.tinfo.size(ti) == bits - mbits:
                ok, count = idaapi.TERR_OK, count + 1

            # Otherwise, we're going to be scanning for how to shrink the type.
            # So, we need to grab the most recent copy of the udt details.
            elif not ti.get_udt_details(utd):
                raise E.DisassemblerError(u"{:s}.remove_slice({:#x}, {!s}{:s}) : Unable to get the details for the specified {:s} type.".format('.'.join([__name__, cls.__name__]), sid, slice, offset_description, type_description))
            ...
            # directly. So, we scan forward until we get to a real member.
                ...
                while index < utd.size() and utd[index].is_gap():
                    index += 1
                logging.debug(u"{:s}.remove_slice({:#x}, {!s}{:s}) : Adjusted index ({:d}) for reducing {:s} from member \"{:s}\" ({:#x}) at {:s} to index {:d}.".format('.'.join([__name__, cls.__name__]), sid, slice, offset_description, mindex, type_description, utils.string.escape(mname, '"'), mid, location_description, index))
                ok = ti.expand_udt(index, -mbits // 8)
            ...
            if ok != idaapi.TERR_OK:
                errname, errdesc = interface.tinfo.format_type_error(ok)
                description = "{:s} ({:s})".format(errname, errdesc) if errname and errdesc else errname if errname else "({:d})".format(terr)
                logging.warning(u"{:s}.remove_slice({:#x}, {!s}{:s}) : Unable to remove space ({:d}) from {:s} after removing member \"{:s}\" ({:#x}) at {:s} due to error {!s}.".format('.'.join([__name__, cls.__name__]), sid, slice, offset_description, mbits, type_description, utils.string.escape(mname, '"'), mid, location_description, description))
                ok, _ = idaapi.TERR_OK, failed.add((mid, mbitoffset))
            continue

        # If we tried to remove the elements but could not, then complain about
        # it via a fatal log and then return the elements we removed (none).
        if selected and not count:
            logging.fatal(u"{:s}.remove_slice({:#x}, {!s}{:s}) : Unable to remove {:d} member{:s} from the specified {:s} ({:#x}).".format('.'.join([__name__, cls.__name__]), sid, slice, offset_description, len(selected), '' if len(selected) == 1 else 's', type_description, sid))
            return []

        # After the members have been removed, the disassembler will not update
        # the changed references immediately. This was why we collected them
        # earlier so that we can ensure that the references are updated.
        ...
        # If the number of items removed matches what we expect, then we're
        # pretty much done here and can return what was removed.
        if is_union:
            iterable = (members[mindex] for mindex, mbitoffset, mid in selected if mindex in members)
        else:
            iterable = (members[mbitoffset] for mindex, mbitoffset, mid in selected if mbitoffset in members)

        if len(selected) == count:
            return [(mname, mtype, mlocation, mtypeinfo, mcomments) for mid, mname, mtype, mlocation, mtypeinfo, mcomments in iterable]

        # Otherwise, we need to gather all the members that we were supposed to
        # remove so that we can figure out which deletions were successsful.
        expected = {mid : (mname, mtype, mlocation, mtypeinfo, mcomments) for mid, mname, mtype, mlocation, mtypeinfo, mcomments in iterable}

        # Now we go through each selected member and verifying that it doesn't
        # exist. If it does, then we need to complain about it.
        ...
        for mindex, mbitoffset, mid in selected:
            ...
            _, mname, _, _, _, _ = members[mkey]

            # Check if the member actually exists at the offset or index.
            exists = v9members.has_index(ti, mindex) if is_union else v9members.has_offset(ti, mbitoffset)
            if exists:
                location_description = "index {:d}".format(mindex) if is_union else "offset {:+#x}".format(base + mbitoffset)
                logging.debug(u"{:s}.remove_slice({:#x}, {!s}{:s}) : Unable to remove member \"{:s}\" at {:s} with id ({:#x}) from the specified {:s} ({:#x}).".format('.'.join([__name__, cls.__name__]), sid, slice, offset_description, utils.string.escape(mname, '"'), location_description, mid, type_description, sid))
        ...
        # Finally we can complain about the members that we could not remove and
        # then return everything that we were successful at removing.
        logging.warning(u"{:s}.remove_slice({:#x}, {!s}{:s}) : Unable to remove {:d} of {:s} from the specified {:s} ({:#x}).".format('.'.join([__name__, cls.__name__]), sid, slice, offset_description, len(expected) - len(removed), "{:d} members".format(len(expected)) if len(expected) == 1 else "the expected {:d} members".format(len(expected)), type_description, sid))
        iterable = (mindex for mindex, mbitoffset, mid in selected if mid in removed) if is_union else (mbitoffset for mindex, mbitoffset, mid in selected if mid in removed)
    ...
    def remove_bounds(cls, type, start, stop, *offset):
        ...
        if not union(ti):
            ...
            mleft = cls.by(ti, lindex) if lindex < utd.size() else ()
        ...
        for mid, mname, mindex, mbitoffset, mbits in listable:
            ...
            # If we didn't error, but there is still a member at the given
            # offset. Then something unexpected happened and we need to fail.
            ...
            # Check if the type was removed exactly the number of bits due to
            # the removal of the member. Because if so, there is nothing to do.
            elif 8 * interface.tinfo.size(ti) == bits - mbits:
                ok, count = idaapi.TERR_OK, count + 1

            # Otherwise, we'll need to scan for gaps to figureo ut how to shrink
            # the type. So, we'll grab the udt details ahead of time here.
            elif not ti.get_udt_details(utd):
                raise E.DisassemblerError(u"{:s}.remove_bounds({:#x}, {:#x}, {:#x}{:s}) : Unable to get the details for the specified {:s} type.".format('.'.join([__name__, cls.__name__]), sid, start, stop, offset_description, type_description))
            ...
            # directly. So, we scan forward until we get to a real member.
                ...
                ok = ti.expand_udt(index, -mbits // 8)  # XXX: bytes
                count = count + 1 if ok == idaapi.TERR_OK else count
            ...
            if ok != idaapi.TERR_OK:
                errname, errdesc = interface.tinfo.format_type_error(ok)
                description = "{:s} ({:s})".format(errname, errdesc) if errname and errdesc else errname if errname else "({:d})".format(terr)
                logging.warning(u"{:s}.remove_bounds({:#x}, {:#x}, {:#x}{:s}) : Unable to remove space ({:d}) from {:s} after removing member \"{:s}\" ({:#x}) at {:s} due to error {!s}.".format('.'.join([__name__, cls.__name__]), sid, start, stop, offset_description, mbits, type_description, utils.string.escape(mname, '"'), mid, location_description, description))
                ok, _ = idaapi.TERR_OK, failed.add((mid, mbitoffset))
            continue
        ...
        if is_union:
            iterable = (members[mindex] for mindex, _, _ in selected if mindex in members)
        else:
            iterable = (members[mbitoffset] for _, mbitoffset, _ in selected if mbitoffset in members)
        ...
        iterable = (mindex for mindex, _, mid in selected if mid in removed) if is_union else (moffset for _, moffset, mid in selected if mid in removed)
    ...
    def clear_slice(cls, type, slice, *offset):
        ...
        for mid, mname, mindex, mbitoffset, mbits in listable:
            ...
            # If we couldn't delete it, then log a warning and track it.
            if ok != idaapi.TERR_OK:
                errname, errdesc = interface.tinfo.format_type_error(ok)
                description = "{:s} ({:s})".format(errname, errdesc) if errname and errdesc else errname if errname else "({:d})".format(terr)
            ...
            # If we succeeded, then we check to see if there's still a member
            # there. If there is, then we were lied to since it wasn't deleted.
            elif not is_union and v9members.has_offset(ti, mbitoffset) and not(next(cls.at_offset(ti, mbitoffset))[-1].is_gap()):
        ...
        for mindex, mbitoffset, mid in selected:
            ...
            mid, mname, _, _, _, _ = members[mkey]
    ...
    def clear_bounds(cls, type, start, stop, *offset):
        ...
        if isinstance(type, idaapi.tinfo_t):
            ...
            raise E.InvalidParameterError(u"{:s}.clear_bounds({!s}, {:#x}, {:#x}{:s}) : Unable to locate the type using an unsupported parameter type ({!s}).".format('.'.join([__name__, cls.__name__]), "{!r}".format(type), start, stop, ", {:+#x}".format(*offset) if offset else '', type.__class__))
        is_variable, is_frame = ti.is_varstruct(), ti.is_frame() if hasattr(ti, 'is_frame') else False

        # Get some descriptions that we will use for exceptions and logging.
        type_description = 'frame' if is_frame else 'union' if union(ti) else 'structure'
        offset_description = ", {:+#x}".format(*offset) if offset else ''

        # Fetch the details for the type and its size.
        utd = idaapi.udt_type_data_t()
        if not ti.get_udt_details(utd):
            raise E.DisassemblerError(u"{:s}.clear_bounds({:#x}, {:#x}, {:#x}{:s}) : Unable to get the details for the specified {:s} type.".format('.'.join([__name__, cls.__name__]), sid, start, stop, offset_description, type_description))
        size, is_union = interface.tinfo.size(ti), union(ti)
        ...
        # If we have a union type, then we need to get the indices in reverse
        ...
        # we complain about it and return our result here.
        if selected and not count:
            bounds = interface.bounds_t(soff, eoff)
            logging.fatal(u"{:s}.clear_bounds({:#x}, {:#x}, {:#x}{:s}) : Unable to remove {:d} member{:s} within the determined boundaries ({:s}) from the specified {:s} ({:#x}).".format('.'.join([__name__, cls.__name__]), sid, start, stop, offset_description, len(selected), '' if len(selected) == 1 else 's', base + bounds, type_description, sid))
            return []
        ...
        for mindex, mbitoffset, mid in selected:
            ...
            if mid in failed:
                logging.warning(u"{:s}.clear_bounds({:#x}, {:#x}, {:#x}{:s}) : Unable to remove member \"{:s}\" ({:#x}) that was at {:s} of {:s} ({:#x}).".format('.'.join([__name__, cls.__name__]), sid, start, stop, offset_description, utils.string.escape(mname, '"'), mid, location_description, type_description, sid))
                mreferences = []
            ...
            # First we log all of the code references.
            if crefs:
                logging.warning(u"{:s}.clear_bounds({:#x}, {:#x}, {:#x}{:s}) : Removal of member \"{:s}\" ({:#x}) at {:s} has resulted in the removal of {:d} reference{:s} ({:s}).".format('.'.join([__name__, cls.__name__]), sid, start, stop, offset_description, utils.string.escape(mname, '"'), mid, location_description, len(crefs), '' if len(crefs) == 1 else 's', ', '.join(map("{:#x}".format, crefs))))
...
class members_t(object):
    ...
    def has(self, name, *suffix):
        ...
        if isinstance(owner.ptr, idaapi.tinfo_t):
            return v9members.has_name(owner.ptr, tuple(itertools.chain(string, suffix)))
    ...
    def has(self, location):
        ...
        # Then we collect all the candidate members that overlap with the
        # realoffset we were given, and get the location of the realoffset with
        # regards to the member. If the offset is exactly at the beginning of
        # the member or any elements of the member, then grab their index.
        if isinstance(owner.ptr, idaapi.tinfo_t):
            candidates = (midx for sptr, midx, _ in v9members.at_offset(owner.ptr, 8 * realoffset))
            ...
            # Next we get the type for each member, and use that to figure out
            ...
            for midx in filtered:
                ...
                # Now we need to determine the boundaries that our location
                # needs to reside in. If it's a variable-length structure and
                # the member has no size, then anything larger than the array
                # could match.
                left, right = realoffset, realoffset + melement * index
                is_variable = tinfo.is_varstruct() and mbits == 0
        ...
        # Otherwise we can use it to get the list of candidates, then we can
        # filter out those that don't align with the realoffset we were given.
    ...
    def has(self, offset):
        ...
        if isinstance(owner.ptr, idaapi.tinfo_t):
            return v9members.has_offset(owner.ptr, 8 * realoffset)
    ...
    def has(self, structure):
        ...
        # If we're backed by an `idaapi.tinfo_t`, then we need to use a
    ...
    def by_name(self, name, *suffix):
        ...
        sptr, mindex, mptr = v9members.by_name(owner.ptr, res) if isinstance(owner.ptr, idaapi.tinfo_t) else members.by_name(owner.ptr, res)
    ...
    def by_offset(self, offset):
        ...
        if isinstance(owner.ptr, idaapi.tinfo_t):
            iterable = v9members.in_offset(owner.ptr, 8 * realoffset)
        else:
            iterable = members.in_offset(owner.ptr, realoffset)
        candidates = [item for item in iterable]
        ...
        left, description = self.baseoffset, "{:#x}".format(offset) if isinstance(offset, types.integer) else "{!s}".format(realoffset)

        # Use the backing type to figure out the namespace we need to use.
        if isinstance(owner.ptr, idaapi.tinfo_t):
            contained = v9members.contains(owner.ptr, 8 * realoffset)
            sid = interface.tinfo.identifier(owner.ptr)
            size = interface.tinfo.size(owner.ptr)
        else:
            contained = members.contains(owner.ptr, realoffset)
            sid = owner.ptr.id
            size = idaapi.get_struc_size(owner.ptr)

        # Figure out which error prevented us from getting the specified member.
        if not contained:
            raise E.OutOfBoundsError(u"{:s}({:#x}).members.by_offset({:s}) : Requested offset ({:+#x}) is outside of the {:s} ({:#x}..{:#x}).".format('.'.join([__name__, cls.__name__]), sid, description, int(offset), 'union' if union(owner.ptr) else 'frame' if frame(owner.ptr) else 'structure', left, left + size - 1))
        elif not size:
            raise E.OutOfBoundsError(u"{:s}({:#x}).members.by_offset({:s}) : Requested offset ({:+#x}) cannot exist within the {:s} due to its size ({:d}).".format('.'.join([__name__, cls.__name__]), sid, description, int(offset), 'union' if union(owner.ptr) else 'frame' if frame(owner.ptr) else 'structure', size))
        raise E.MemberNotFoundError(u"{:s}({:#x}).members.by_offset({:s}) : Unable to locate a member at the specified offset ({:+#x}) of the {:s}.".format('.'.join([__name__, cls.__name__]), sid, description, int(offset), 'union' if union(owner.ptr) else 'frame' if frame(owner.ptr) else 'structure'))
    ...
    def by_realoffset(self, offset):
        ...
        left, description = self.baseoffset, "{:#x}".format(offset) if isinstance(offset, types.integer) else "{!s}".format(offset)

        # Next we need to figure out the backing type so that we can choose the
        # correct namespace and apis to use.
        if isinstance(owner.ptr, idaapi.tinfo_t):
            contained = v9members.contains(owner.ptr, 8 * realoffset)
            sid = interface.tinfo.identifier(owner.ptr)
            size = interface.tinfo.size(owner.ptr)
        ...
        if not contained:
            raise E.MemberNotFoundError(u"{:s}({:#x}).members.by_realoffset({:s}) : Unable to locate a member at the specified offset ({:+#x}) of the {:s}.".format('.'.join([__name__, cls.__name__]), sid, description, int(offset), 'union' if union(owner.ptr) else 'frame' if frame(owner.ptr) else 'structure'))
        elif not size:
            raise E.OutOfBoundsError(u"{:s}({:#x}).members.by_realoffset({:s}) : Requested offset ({:+#x}) cannot exist within the {:s} due to its size ({:d}).".format('.'.join([__name__, cls.__name__]), sid, description, int(offset), 'union' if union(owner.ptr) else 'frame' if frame(owner.ptr) else 'structure', size))
        raise E.OutOfBoundsError(u"{:s}({:#x}).members.by_realoffset({:s}) : Requested offset ({:+#x}) is outside of the {:s} ({:#x}..{:#x}).".format('.'.join([__name__, cls.__name__]), sid, description, int(offset), 'union' if union(owner.ptr) else 'frame' if frame(owner.ptr) else 'structure', left, left + size - 1))
    ...
    def by_identifier(self, id):
        ...
        owner = self.owner
        _, mindex, _ = v9members.by_identifier(owner.ptr, id) if isinstance(owner.ptr, idaapi.tinfo_t) else members.by_identifier(owner.ptr, id)
        return member_t(owner, mindex)
    ...
    def index(self, member):
        ...
        if not hasattr(member, 'id'):
            raise E.InvalidParameterError(u"{:s}({:#x}).members.index({!r}) : An invalid type ({!s}) was specified as the member being requested.".format('.'.join([__name__, cls.__name__]), sid, member, member.__class__))
        ...
        # member so that we can customize the exception that we re-raise.
    ...
    def pop(self, index):
        ...
        if isinstance(owner.ptr, idaapi.tinfo_t):
            results = v9members.remove_slice(owner.ptr, index, base)
        else:
    ...
    def add(self, name, type):
        ...
        owner = self.owner
        is_union = union(owner.ptr)

        # If we're using an `idaapi.tinfo_t` as our backing type, then we need
        # to use the correct namespaces. If it's a union, then we use the number
    ...
    def add(self, name, type, offset):
        ...
        elif isinstance(owner.ptr, idaapi.tinfo_t):
            bitoffset = 8 * offset
            if frame(owner.ptr):
                mowner, mindex, mptr = v9members.add(owner.ptr, name, type, bitoffset, owner.baseoffset)
            else:
                mowner, mindex, mptr = v9members.add(owner.ptr, name, type, bitoffset, owner.baseoffset)
            return member_t(owner, mindex)

        # We explicitly trust whatever offset (in bytes) that the user gives us
        # for frames or regular structure-like types.
        elif frame(sid):
    ...
    def remove(self, offset):
        ...
        if isinstance(owner.ptr, idaapi.tinfo_t):
            items = [packed for packed in v9members.at_offset(owner.ptr, 8 * realoffset)]
        else:
            items = [packed for packed in members.at_offset(owner.ptr, realoffset)]
        ...
        if not items:
            raise E.MemberNotFoundError(u"{:s}({:#x}).members.remove({:+#x}) : Unable to find a member at the specified offset ({:#x}) of the {:s} ({:s}).".format('.'.join([__name__, cls.__name__]), sid, offset, offset, 'union' if union(owner.ptr) else 'frame' if frame(owner.ptr) else 'structure', owner.bounds))

        elif len(items) > 1:
            raise E.InvalidTypeOrValueError(u"{:s}({:#x}).members.remove({:+#x}) : Refusing to remove more than {:d} member{:s} ({:d}) at offset {:#x}.".format('.'.join([__name__, cls.__name__]), sid, offset, 1, '' if len(items) == 1 else 's', len(items), offset))
    ...
    def ptr(self):
        ...
        # FIXME: this property should probably be removed.
        return self
    ...
    __members_matcher.mapping('named', operator.truth, utils.fcondition(utils.fcompose(operator.attrgetter('parent'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9member.has_name), utils.fcompose(operator.attrgetter('ptr'), member.has_name)))
    __members_matcher.combinator('tagged', utils.fcompose(utils.fcompose, utils.fcondition(utils.finstance(internal.types.bool, internal.types.integer), utils.finstance(internal.types.string))(utils.fcondition(operator.truth)(utils.fcompose(utils.fdiscard(internal.tags.select.members), utils.fpartial(utils.imap, operator.itemgetter(0)), internal.types.set, utils.fpartial(utils.fpartial, operator.contains)), utils.fcompose(utils.fdiscard(internal.tags.select.members), utils.fpartial(utils.imap, operator.itemgetter(0)), internal.types.set, utils.fpartial(utils.fpartial, utils.fcompose(operator.contains, operator.not_)))), utils.fcompose(internal.tags.select.members, utils.fpartial(utils.itermap, operator.itemgetter(0)), internal.types.set, utils.fpartial(utils.fpartial, operator.contains)), utils.fcompose(utils.fthrough(utils.fconstant('include'), internal.types.set), utils.fpack(internal.types.list), internal.types.dictionary, utils.fkeyword(internal.tags.select.members), utils.fpartial(utils.itermap, operator.itemgetter(0)), internal.types.set, utils.fpartial(utils.fpartial, operator.contains)))), 'id')
    __members_matcher.alias('tag', 'tagged'), __members_matcher.alias('tags', 'tagged')
    __members_matcher.combinator('type', utils.fcondition(utils.finstance(internal.types.bool, internal.types.integer), utils.finstance(idaapi.tinfo_t), utils.finstance(structuretypes), utils.finstance(internal.types.unordered))(utils.fcompose(utils.fcompose(operator.truth, utils.fpartial(utils.fpartial, operator.eq)), utils.fpartial(utils.fcompose, utils.fcondition(utils.fcompose(operator.attrgetter('parent'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9member.has_typeinfo), utils.fcompose(operator.attrgetter('ptr'), member.has_typeinfo)))), utils.fcompose(utils.fpartial(utils.fpartial, interface.tinfo.equals), utils.fpartial(utils.fcompose, utils.fcondition(utils.fcompose(operator.attrgetter('parent'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9member.get_typeinfo), utils.fcompose(operator.attrgetter('ptr'), member.get_typeinfo)))), utils.fcompose(utils.fcondition(utils.fcompose(utils.fthrough(utils.finstance(structure_t), utils.fcompose(operator.attrgetter('owner'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t))), all))(operator.attrgetter('ptr'), utils.fcompose(operator.attrgetter('id'), address.type)), utils.fpartial(utils.fpartial, interface.tinfo.equals), utils.fpartial(utils.fcompose, utils.fcondition(utils.fcompose(operator.attrgetter('parent'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9member.get_typeinfo), utils.fcompose(operator.attrgetter('ptr'), member.get_typeinfo)))), utils.fcompose(utils.fpartial(utils.itermap, utils.fcondition(utils.finstance(structuretypes), utils.finstance(idaapi.tinfo_t))(utils.fcondition(utils.fcompose(utils.fthrough(utils.finstance(structure_t), utils.fcompose(operator.attrgetter('owner'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t))), utils.funpack(operator.and_)))(operator.attrgetter('ptr'), utils.fcompose(operator.attrgetter('id'), address.type)), utils.fidentity, utils.fcompose(utils.fthrough(utils.fcompose(operator.attrgetter('__class__'), operator.attrgetter('__name__')), utils.fidentity), utils.funpack(utils.fpartial("{:s} : Unable to match the requested filter (\"{:s}\") with an unsupported type \"{:s}\" ({!r}).".format, '.'.join([__name__, 'members_t']), 'type')), utils.fthrow(E.InvalidMatchTypeError)))), utils.fpartial(utils.iterfilter, None), utils.fpartial(utils.itermap, utils.fcompose(utils.fpartial(utils.fpartial, interface.tinfo.equals), utils.fpartial(utils.fcompose, utils.fcondition(utils.finstance(structure_t))(utils.fcondition(utils.fcompose(operator.attrgetter('owner'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9member.get_typeinfo), utils.fcompose(operator.attrgetter('ptr'), member.get_typeinfo)), utils.fcompose(operator.attrgetter('id'), address.type))))), utils.funpack(utils.fthrough), utils.frpartial(utils.fcompose, builtins.any)), utils.fcompose(utils.fthrough(utils.fcompose(operator.attrgetter('__class__'), operator.attrgetter('__name__')), utils.fidentity), utils.funpack(utils.fpartial("{:s} : Unable to match the requested filter (\"{:s}\") with an unsupported type \"{:s}\" ({!r}).".format, '.'.join([__name__, 'members_t']), 'type')), utils.fthrow(E.InvalidMatchTypeError))))
    ...
    __members_matcher.combinator('structure', utils.fcondition(utils.finstance(internal.types.string), utils.finstance(internal.types.integer), utils.finstance(structuretypes), utils.finstance(idaapi.tinfo_t), utils.finstance(internal.types.unordered))(utils.fcompose( utils.fcompose(utils.string.to, idaapi.get_struc_id) if idaapi.__version__ < 8.4 else utils.fcompose(interface.tinfo.for_name, interface.tinfo.identifier), utils.fcondition(utils.fpartial(operator.ne, idaapi.BADADDR))(utils.fpartial(utils.fpartial, operator.eq), utils.fpartial(utils.fpartial, utils.fconstant(False)))), utils.fpartial(utils.fpartial, operator.eq), utils.fcompose(operator.attrgetter('id'), utils.fpartial(utils.fpartial, operator.eq)), utils.fcompose(utils.fcatch(None)(None)(interface.tinfo.structure), utils.fcondition(utils.fpartial(operator.ne, None))(utils.fcompose(operator.methodcaller('get_type_name'), idaapi.get_struc_id) if idaapi.__version__ < 8.4 else interface.tinfo.identifier, utils.fconstant(idaapi.BADADDR)), utils.fpartial(utils.fpartial, operator.eq)), utils.fcompose(utils.fpartial(utils.itermap, utils.fcondition(utils.finstance(internal.types.string), utils.finstance(internal.types.integer), utils.finstance(structuretypes), utils.finstance(idaapi.tinfo_t))(utils.fcompose(utils.fcompose(utils.string.of, idaapi.get_struc_id) if idaapi.__version__ < 8.4 else utils.fcompose(interface.tinfo.for_name, interface.tinfo.identifier), utils.fcondition(utils.fpartial(operator.ne, idaapi.BADADDR))(utils.fidentity, utils.fpartial(utils.fpartial, utils.fconstant(None)))), utils.fidentity, operator.attrgetter('id'), utils.fcompose(utils.fcatch(None)(None)(interface.tinfo.structure), utils.fcondition(utils.fpartial(operator.ne, None))(utils.fcompose(operator.methodcaller('get_type_name'), idaapi.get_struc_id) if idaapi.__version__ < 8.4 else interface.tinfo.identifier, utils.fidentity)), utils.fcompose(utils.fthrough(utils.fcompose(operator.attrgetter('__class__'), operator.attrgetter('__name__')), utils.fidentity), utils.funpack(utils.fpartial("{:s} : Unable to match the requested filter (\"{:s}\") with an unsupported type \"{:s}\" ({!r}).".format, '.'.join([__name__, 'members_t']), 'structure')), utils.fthrow(E.InvalidMatchTypeError)))), utils.fpartial(filter, utils.fpartial(operator.ne, None)), internal.types.set, utils.fpartial(utils.fpartial, operator.contains)), utils.fcompose(utils.fthrough(utils.fcompose(operator.attrgetter('__class__'), operator.attrgetter('__name__')), utils.fidentity), utils.funpack(utils.fpartial("{:s} : Unable to match the requested filter (\"{:s}\") with an unsupported type {:s} ({!r}).".format, '.'.join([__name__, 'members_t']), 'structure')), utils.fthrow(E.InvalidMatchTypeError))), utils.fcondition(utils.fcompose(operator.attrgetter('parent'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9member.get_typeinfo), utils.fcompose(operator.attrgetter('ptr'), member.get_typeinfo)), utils.fcatch(None)(None)(interface.tinfo.structure), utils.fcondition(utils.finstance(idaapi.tinfo_t))(utils.fcompose(operator.methodcaller('get_type_name'), idaapi.get_struc_id) if idaapi.__version__ < 8.4 else interface.tinfo.identifier, idaapi.BADADDR))
    __members_matcher.alias('structures', 'structure'), __members_matcher.alias('struc', 'structure'),
    __members_matcher.mapping('arguments', operator.truth, utils.fcondition(utils.fcompose(operator.attrgetter('id'), has_member))(utils.fcompose(utils.fthrough(utils.fcompose(utils.fcondition(utils.fcompose(operator.attrgetter('parent'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9members.by, operator.itemgetter(0)), utils.fcompose(operator.attrgetter('id'), idaapi.get_member_by_id, operator.itemgetter(-1))) if hasattr(idaapi, 'get_member_by_id') else utils.fcompose(operator.attrgetter('id'), v9members.by, operator.itemgetter(0)), interface.function.by_frame, utils.fcondition(bool)(utils.fcompose(idaapi.frame_off_args, functools.partial(functools.partial, operator.le)), functools.partial(functools.partial, utils.fconstant(False)))), utils.fcondition(utils.fcompose(operator.attrgetter('parent'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9members.by, operator.itemgetter(-1), operator.attrgetter('offset'), lambda bits: bits // 8), utils.fcompose(operator.attrgetter('ptr'), operator.attrgetter('soff')))), utils.funpack(utils.fapplyto())), utils.fconstant(False)))
    __members_matcher.alias('args', 'arguments'), __members_matcher.alias('parameters', 'arguments'),
    __members_matcher.mapping('locals', operator.truth, utils.fcondition(utils.fcompose(operator.attrgetter('id'), has_member))(utils.fcompose(utils.fthrough(utils.fcompose(utils.fcondition(utils.fcompose(operator.attrgetter('parent'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9members.by, operator.itemgetter(0)), utils.fcompose(operator.attrgetter('id'), idaapi.get_member_by_id, operator.itemgetter(-1)) if hasattr(idaapi, 'get_member_by_id') else utils.fcompose(operator.attrgetter('id'), v9members.by, operator.itemgetter(0))), interface.function.by_frame, utils.fcondition(bool)(utils.fcompose(idaapi.frame_off_savregs, functools.partial(functools.partial, operator.gt)), functools.partial(functools.partial, utils.fconstant(False)))), utils.fcondition(utils.fcompose(operator.attrgetter('parent'), operator.attrgetter('ptr'), utils.finstance(idaapi.tinfo_t)))(utils.fcompose(operator.attrgetter('id'), v9members.by, operator.itemgetter(-1), operator.attrgetter('offset'), lambda bits: bits // 8), utils.fcompose(operator.attrgetter('ptr'), operator.attrgetter('soff')))), utils.funpack(utils.fapplyto())), utils.fconstant(False)))
    ...
    def __str__(self):
        ...
        for sptr, mindex, mptr in iterable:
            ...
            res.append((mindex, name, pythonic, typeinfo, moffset, msize, comment or '', tag))
            mn = max(mn, len(name))
            ms = max(ms, len("{:+#x}".format(moffset)))
            ms = max(ms, len("{:+#x}".format(msize)))
            mti = max(mti, len("{!s}".format(typeinfo.dstr()).replace(' *', '*')))
            eoff = moffset + msize
        ...
        if len(self):
            ...
            return "{!r}\n{:s}".format(self.owner, '\n'.join("{:<{:d}s} {:>{:d}x}{:<+#{:d}x} {:>{:d}s} {:<{:d}s} {!s}{:s}".format('' if i < 0 else "[{:d}]".format(i), 2 + mi, base + o, mo, s, ms, "{!s}".format(ti.dstr()).replace(' *','*') if ti else '', mti, '' if i < 0 else utils.string.repr(n), mn + 2, utils.string.repr(t), " // {!s}".format(utils.string.repr(T) if '\n' in c else utils.string.to(c)) if c else '') for i, n, t, ti, o, s, c, T in res))
    ...
    def __unicode__(self):
        ...
        for sptr, mindex, mptr in members.iterate(owner.ptr):
            ...
            ms = max(ms, len("{:+#x}".format(msize)))
            mti = max(mti, len("{!s}".format(typeinfo.dstr()).replace(' *', '*')))
            eoff = base + moffset + msize

        mi = len("{:d}".format(len(self) - 1)) if len(self) else 1

        if len(self):
            mo = max(map(len, map("{:x}".format, (self.baseoffset, self[-1].offset + self[-1].size))))
            return u"{!r}\n{:s}".format(self.owner, '\n'.join("{:<{:d}s} {:>{:d}x}{:<+#{:d}x} {:>{:d}s} {:<{:d}s} {!s}{:s}".format('' if i < 0 else "[{:d}]".format(i), mi, base + o, mo, s, ms, "{!s}".format(ti.dstr()).replace(' *','*') if ti else '', mti, '' if i < 0 else utils.string.repr(n), mn + 2, utils.string.repr(t), " // {!s}".format(utils.string.repr(T) if '\n' in c else utils.string.to(c)) if c else '') for i, n, t, ti, o, s, c, T in res))
    ...
    def __len__(self):
        ...
        owner = self.owner
        if isinstance(owner.ptr, idaapi.tinfo_t):
            return v9members.count(owner.ptr)
    ...
    def __getitem__(self, index):
        ...
        # Check the backing type to figure out which namespace we should use.
        if isinstance(owner.ptr, idaapi.tinfo_t):
            membersnamespace = v9members
        else:
            membersnamespace = members

        # Return the element at the specified index.
        if isinstance(index, types.integer):
            index = membersnamespace.count(owner.ptr) + index if index < 0 else index
            _, mindex, _ = membersnamespace.by_index(owner.ptr, index)
            res = member_t(owner, mindex)

        # Return the element with the specified name.
        elif isinstance(index, types.string):
            _, mindex, _ = membersnamespace.by_name(owner.ptr, index)
            res = member_t(owner, mindex)

        # Return the elements for the specified slice.
        ...
        if res is None:
            where = "with the specified name (\"{:s}\")".format(index) if isinstance(index, types.string) else "at the given index ({:d})".format(index)
            raise E.MemberNotFoundError(u"{:s}({:#x}).members.__getitem__({:d}) : Unable to find a member {:s}.".format('.'.join([__name__, cls.__name__]), sid, index, where))
        return res
