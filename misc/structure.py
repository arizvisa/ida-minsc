...
class member(object):
    ...
    def set_typeinfo(cls, mptr, info, flags=idaapi.SET_MEMTI_COMPATIBLE):
        ...
        if res == idaapi.SMT_OK or res == idaapi.SMT_KEEP and interface.tinfo.equals(idaapi.get_idati(), ti, prevti):
            return original
    ...
    def get_comment(cls, mptr, *repeatable):
        ...
        """
        res = idaapi.get_member_cmt(mptr.id, *repeatable) if repeatable else idaapi.get_member_cmt(mptr.id, True) or idaapi.get_member_cmt(mptr.id, False)
    ...
    def set_comment(cls, mptr, string, *repeatable):
        ...
        return utils.string.to(result or '')
    ...
    def references(cls, mptr):
        ...
        packed = idaapi.get_member_by_id(identifier)
        if not packed:
            raise E.MemberNotFoundError(u"{:s}.refs({:#x}) : Unable to locate the member using the specified identifier ({:#x}).".format('.'.join([__name__, cls.__name__]), identifier, identifier))
        mptr, fullname, owner = packed
        ...
        if interface.node.identifier(owner.id) and is_frame and fn != idaapi.BADADDR:
            ...
            for ea, iscode, xtype in interface.xref.to(mptr.id, idaapi.XREF_ALL):
                ...
                if not listable:
                    logging.debug(u"{:s}.refs({:#x}) : Skipping reference to member ({:#x}) at {:#x} with flags ({:#x}) due to no operand information.".format('.'.join([__name__, cls.__name__]), mptr.id, mptr.id, ea, address.flags(ea)))
                ...
                # use the structure path to find the operand that exists.
                    ...
                    logging.debug(u"{:s}.refs({:#x}) : Found strpath_t to member ({:#x}) at {:#x} with flags ({:#x}).".format('.'.join([__name__, cls.__name__]), mptr.id, mptr.id, ea, address.flags(ea)))
                ...
                # there really shouldn't be any other operand flag for a stkvar.
                    ...
                    logging.warning(u"{:s}.refs({:#x}) : Skipping reference to member ({:#x}) at {:#x} with flags ({:#x}) due to the operand type being unexpected.".format('.'.join([__name__, cls.__name__]), mptr.id, mptr.id, ea, address.flags(ea)))
                continue
            return results
        ...
        for ea, iscode, xtype in refs:
            ...
            if flags & FF_STKVAR in {FF_STKVAR, idaapi.FF_0STK, idaapi.FF_1STK}:
                logging.debug(u"{:s}.refs({:#x}) : Found stkvar_t to member ({:#x}) at {:#x} with flags ({:#x}).".format('.'.join([__name__, cls.__name__]), mptr.id, mptr.id, ea, address.flags(ea)))
            ...
            # filter these by only taking the ones which we can use to calculate the target.
                ...
                logging.debug(u"{:s}.refs({:#x}) : Found refinfo_t to member ({:#x}) at {:#x} with flags ({:#x}).".format('.'.join([__name__, cls.__name__]), mptr.id, mptr.id, ea, address.flags(ea)))
...
class members(object):
    ...
    def index(cls, sptr, mptr):
        ...
        if union(sptr) and mptr.flag & idaapi.MF_UNIMEM:
            return mptr.soff
    ...
    def by_identifier(cls, sptr, identifier):
        ...
        sptr = sptr_ if interface.node.identifier(sptr.id) else idaapi.get_member_struc(utils.string.to(fullname))
        base = idaapi.get_prev_member_idx(sptr, sptr.members[0].eoff)
    ...
    def references(cls, sptr):
        ...
        for xrfrom, xriscode, xrtype in refs:
            ...
            if not address.code(xrfrom):
                logging.debug(u"{:s}.refs({:#x})) : Skipping {:s} reference at {:#x} with the type ({:d}) due to its address not being marked as code.".format('.'.join([__name__, cls.__name__]), sptr.id, 'code' if xriscode else 'data', xrfrom, xrtype))
                continue
    ...
    def layout_setslice(cls, sptr, slice, layout, *offset):
        ...
        newitems, area_t, iterable = [], idaapi.area_t if idaapi.__version__ < 7.0 else idaapi.range_t, interface.contiguous.layout(left, newlayout, +1)
        ...
        for mname, offsets in duplicates.items():
            for offset in offsets:
                ...
                while name in candidates or idaapi.get_member_by_fullname(utils.string.to('.'.join([sname, name]))):
        ...
        for offset, item, packed in newitems:
            ...
            error_description[idaapi.STRUC_ERROR_MEMBER_TINFO] = 'an invalid type id', "{:#x}".format(opinfo.tid)
...
class members_t(object):
    ...
    def has(self, structure):
        ...
        til, sptr = idaapi.get_idati(), structure if isinstance(structure, idaapi.struc_t) else structure.ptr
        ...
        for midx in range(owner.memqty):
            ...
            if candidate and interface.tinfo.equals(til, stype, candidate):
                return True
            continue
        return False
    ...
    def has(self, info):
        ...
        til, owner = idaapi.get_idati(), self.owner.ptr
        for midx in range(owner.memqty):
            mptr = owner.get_member(midx)
            mtype = address.type(mptr.id)
            if mtype is not None and interface.tinfo.equals(til, mtype, info):
                return True
            continue
        return False
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, types.integer):
            ...
            sliced = members.slice(owner.ptr, index)
            res = [(owner_index_member if isinstance(owner_index_member, types.integer) else member_t(owner, owner_index_member[1])) for owner_index_member in sliced]
